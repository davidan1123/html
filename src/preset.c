...
preset_notify(int count)
	...
	if (!event)
		event_count_down_add("preset notify", count,
				event_notify_expire, &dav.preset_notify);
	dav.preset_notify = TRUE;
	}
...
preset_find_at_position(int pan, int tilt)
	...
	for (list = dav.preset_position_list; list; list = list->next)
		...
		if (pos->pan == pan && (tilt == 0 || tilt == pos->tilt))
			{
			dav.preset_position_index = index;
			break;
			}
		++index;
		pos = NULL;
...
preset_position_prev(void)
	...
	if (idx >= dav.n_preset_positions)
		idx = dav.n_preset_positions - 1;
	if (idx == -1)
		idx = 0;
	return idx;
	}
...
//dav.have_servos, do_pan,
//dav.preset_position_index, pos->settings_index, pos->pan, pos->tilt);

	if (dav.have_servos && do_pan && pos->pan > 0)
		{
		event_list_lock();
		if ((event = event_find("preset notify")) != NULL)
			if (event->count > 0)
				event->count = 1;
		event_list_unlock();
		servo_move(pos->pan, pos->tilt, dav.servo_preset_step_msec);
	...
	if (settings)
		{
		dav.motion_magnitude_limit = settings->mag_limit;
		dav.motion_magnitude_limit_count = settings->mag_limit_count;
		dav.motion_burst_count = settings->burst_count;
		dav.motion_burst_frames = settings->burst_frames;
		dav.zoom_percent = settings->zoom_percent;
...
preset_new(PresetPosition *pos_src)
	...
	if (!pos)
		...
		dav.preset_position_list =
				slist_insert_sorted(dav.preset_position_list, pos,
						pan_position_cmp);
		dav.n_preset_positions += 1;
		dav.preset_position_index =
				slist_index(dav.preset_position_list, pos);
		if (dav.preset_position_index == -1)
			dav.preset_position_index = 0;
		}
	else
		pos_src = NULL;
...
preset_delete(void)
	...
	if (dav.n_preset_positions == 1 && pos->n_settings == 1)
		{
	...
	if (pos->n_settings <= 0)
		...
		dav.preset_position_list = slist_remove(dav.preset_position_list, pos);
		free(pos);
		dav.n_preset_positions -= 1;
		dav.on_preset = FALSE;
		dav.preset_last_on = NULL;
		if (dav.preset_position_index > 0)
...
//      inform_message("Bad preset command.");
	...
	switch (id)
		{
		case PREV_POSITION:
			dav.preset_position_index = preset_position_prev();
			preset_load_values(TRUE);
			break;

		case NEXT_POSITION:
			dav.preset_position_index = preset_position_next();
			preset_load_values(TRUE);
			break;
		...
		case MOVE_ALL:
			move_all = TRUE;
		case MOVE_ONE:
			if (dav.on_preset || !dav.preset_last_on)
				{
				if (dav.on_preset)
			...
			else if (dav.preset_last_on)
				{
				dpan = pan - dav.preset_last_on->pan;
				dtilt = tilt - dav.preset_last_on->tilt;
				for (list = dav.preset_position_list; list; list = list->next)
					{
					pos = (PresetPosition *) list->data;
					if (move_all || pos == dav.preset_last_on)
						{
						if (   pos->pan + dpan > dav.servo_pan_max
						    || pos->pan + dpan < dav.servo_pan_min
						    || pos->tilt + dtilt > dav.servo_tilt_max
						    || pos->tilt + dtilt < dav.servo_tilt_min
						   )
							{
				...
				if (move_all)
					...
					*/
					dav.preset_position_list =
						slist_remove(dav.preset_position_list, pos);
					dav.preset_position_list =
						slist_insert_sorted(dav.preset_position_list,
		...
		case GOTO:
			if (sscanf(arg1, "%d %d", &n, &i) == 2)
				...
				if (--n < 0)	/* n was 1 based position number */
					n = dav.preset_position_index;
				pos = (PresetPosition *) slist_nth_data(dav.preset_position_list, n);
				if (pos)
					{
					dav.preset_position_index = n;
...
preset_config_load(void)
	...
	if ((f = fopen(dav.preset_config_file, "r")) == NULL)
		{
		preset_new(NULL);
		preset_config_save();
		return;
	...
	while (fgets(buf, sizeof(buf), f) != NULL)
		...
		if (sscanf(buf, "<position %d %d>", &pan, &tilt) == 2)
			...
			pos->pan = pan;
			pos->tilt = tilt;
			dav.preset_position_list = slist_append(dav.preset_position_list, pos);
			dav.n_preset_positions += 1;
			continue;
			}
		if (!pos)
...
preset_config_save(void)
	...
	if ((f = fopen(dav.preset_config_file, "w")) == NULL)
		{
		log_printf("Failed to save preset config file %s. %m\n",
				dav.preset_config_file);
		return;
		}
	for (list = dav.preset_position_list; list; list = list->next)
		{
...
preset_state_save(void)
	...
	f = fopen(dav.preset_state_file, "w");
	if (f)
		{
		servo_get_position(&pan, &tilt);
		fprintf(f, "pan %d\n", pan);
...
preset_state_load(void)
	{
	FILE			*f;
	SList			*list = dav.preset_position_list;
	PresetPosition	*pos;
	int				pan = 150, tilt = 150, i;
	char	buf[100];

	if ((f = fopen(dav.preset_state_file, "r")) == NULL)
		{
		preset_load_values(TRUE);
		return;
		}
...
/* XXX */
		...
		if (!strncmp(buf, "pan", 3))
			...
			sscanf(buf + 15, "%d", &dav.preset_position_index);
	...
	fclose(f);
	if (   dav.preset_position_index < 0
	    || dav.preset_position_index > dav.n_preset_positions - 1
	   )
		dav.preset_position_index = 0;
	if (pan < dav.servo_pan_min || pan > dav.servo_pan_max)
		pan = 150;
	if (tilt < dav.servo_tilt_min || tilt > dav.servo_tilt_max)
		tilt = 150;
	...
	*/
	preset_load_values(FALSE);
	if (dav.have_servos)
		servo_move(pan, tilt, dav.servo_move_step_msec);
	else
		preset_notify(22);
	}
