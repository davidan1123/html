...
audio_circular_buffer_init(void)
	...
	acb->channels = dav.audio_channels;
...
//			memset(acb->data, 0, acb->data_size);
			log_printf("audio circular buffer - %.2f KB  %.2f KFrames (%d sec, %d samples/sec, %d channels)\n",
				...
				vcb->seconds, acb->rate, dav.audio_channels);
			}
		else
			{
			acb->data_size = 0;
...
audio_record_write(AudioCircularBuffer *acb, int start_frame, int n_frames)
	...
	if ((dav.audio_debug & 0x1) && debug_wave_file)
...
audio_buffer_set_record_head(AudioCircularBuffer *acb, int head)
	...
	if (dav.audio_debug & 0x4)
		...
		vid_time = (float) (vcb->last_pts - dav.video_start_pts) / 1e6;
...
"REC audio (h:%d t[%d]:%d) audio (frames %d time %.2f)  video (frames %d time %.2f)\n",
				...
				vcb->frame_count,
				(float) (vcb->last_pts - dav.video_start_pts) / 1e6);
		}
...
audio_frames_offset_from_video(AudioCircularBuffer *acb)
	...
	vid_time = (double)(vcb->last_pts - dav.video_start_pts) / 1e6;
...
audio_gain(AudioCircularBuffer *acb, int16_t *pcm_buf, int frames)
	...
	if (peak0 > dav.audio_level)
		dav.audio_level = peak0;
	if (peak1 > dav.audio_level)
		dav.audio_level = peak1;
	}
...
audio_record_start(void)
	...
	acb->mp3_file = fopen(dav.audio_pathname, "w");
	if (!acb->mp3_file)
		{
	...
	quality = (dav.pi_model == 2)
		? dav.audio_mp3_quality_Pi2 : dav.audio_mp3_quality_Pi1;
	lame_set_quality(acb->lame_record, quality);
	lame_init_params(acb->lame_record);

	if (dav.audio_debug & 0x1)
		{
...
audio_record_stop(void)
	...
	audio_record_files_close();
	dav.audio_last_frame_count = acb->record_frame_count;
	dav.audio_last_rate = (int) ((dav.audio_last_frame_count
						/ dav.video_last_time + 0.5));
	}
...
audio_mic_open(boolean inform)
	...
	max_mp3_encode_periods = (dav.pi_model == 1) ? 2 : 3;

	if ((err = snd_pcm_open(&acb->pcm, dav.audio_device,
	...
	*/
	...
	dav.audio_channels = acb->channels;
	...
	gain = powf(10.0, dav.audio_gain_dB / 20.0);
...
audio_fifo_write_open(void *data)
	...
	acb->mp3_stream_fd = open(dav.audio_fifo, O_WRONLY|O_NONBLOCK);
	if (acb->mp3_stream_fd >= 0)
		{
	...
	switch (id)
		...
		case AUDIO_MIC_TOGGLE:
			...
			if (acb->pcm)
				{
				if (audio_mic_close())
					dav.audio_enable = FALSE;
				}
			else
				dav.audio_enable = audio_mic_open(TRUE);

			if (n != dav.audio_enable)
