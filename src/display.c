...
//	uint8_t		*pU, *pV;
	...
	for (y = 0; y < dav.mjpeg_height; ++y)
		{
		y_mv = MJPEG_TO_MOTION_VECTOR_Y(y);
		for (x = 0; x < dav.mjpeg_width; ++x)
			{
			x_mv = MJPEG_TO_MOTION_VECTOR_X(x);
			ptrig = mf->trigger + x_mv + mf->width * y_mv;

			pY = i420 + x + y * dav.mjpeg_width;

//			pV = pY + (((pkrellcam.mjpeg_height + 15) & ~15) * dav.mjpeg_width);
//			pV += (y / 2) * dav.mjpeg_width + x / 2;

//			pU = pY + (((((dav.mjpeg_height * 3) / 2) + 15) & ~15) * dav.mjpeg_width);
//			pU += (y / 2) * dav.mjpeg_width + x / 2;

			Ydim = *pY * dav.motion_vectors_dimming / 100;
			Ytrig = *pY;
			if (Ytrig < 225)
				Ytrig += 30;
			else if (Ytrig < 235)
...
display_preset_setting(void)
	...
	if (dav.on_preset)
		{
		pos = (PresetPosition *) slist_nth_data(dav.preset_position_list, dav.preset_position_index);
...
display_servo_pan(void)
	...
	dx = 2 * (dav.servo_pan_max - dav.servo_pan_min);
	x0 = X_MAP(da, dx);
	if (dav.servo_pan_max < 200)
		x0 -= 2 * (200 - dav.servo_pan_max);
	...
	for (i = 30; i < 270; i += 10)
		...
		else if (i <= dav.servo_pan_min || i >= dav.servo_pan_max)
			continue;
		else
	...
	for (list = dav.preset_position_list; list; list = list->next)
		{
		pos = (PresetPosition *) list->data;
		x = x0 + 2 * (pos->pan - dav.servo_pan_min);
		if (   (pos->pan == pan && pos->tilt == tilt)
		    || pos == dav.preset_last_on
		   )
...
display_servo_tilt(void)
	...
	y0 = (dav.servo_tilt_min > 100) ?
				dav.servo_tilt_min - 100 : 0;
	dy = 2 * (dav.servo_tilt_max - dav.servo_tilt_min);
	...
	if (mf->show_preset || adjustment_is_servo_limits())
		...
		y1 = y;
		if (dav.servo_tilt_max < 199)
			y1 -= 2 * (200 - dav.servo_tilt_max);
		i420_draw_string(da, normal_font, 0xff,
	...
	if (dav.servo_tilt_min > 101)
		{
		glcd_draw_v_line(glcd, da, 0, x, y + dy,
					2 * (dav.servo_tilt_min - 100));
	...
	for (i = 30; i < 270; i += 10)
		{
		if (dav.servo_tilt_min > 100)
			y = i - 100;
		else
			y = i - dav.servo_tilt_min;
		y = Y_MAP(da, 2 * y);
		...
		else if (i <= dav.servo_tilt_min || i >= dav.servo_tilt_max)
			continue;
		else
	...
	pos = (PresetPosition *) slist_nth_data(dav.preset_position_list,
				dav.preset_position_index);
	if (pos)
		{
		if (dav.servo_tilt_min > 100)
			y = pos->tilt - 100;
		else
			y = pos->tilt - dav.servo_tilt_min;
		y = Y_MAP(da, 2 * y);
...
display_preset_settings(void)
	...
	if (dav.preset_modified_warning)
		{
	...
	snprintf(info1, sizeof(info1), "Mag %d  Cnt %d   Burst %d,%d  Zoom %d%%",
				...
				dav.motion_burst_count, dav.motion_burst_frames,
				dav.zoom_percent);
...
display_audio(void)
	...
	snprintf(buf, sizeof(buf), "%ddB", dav.audio_gain_dB);
	...
	if (dav.audio_trigger_video)
		{
		dy = dav.audio_trigger_level * VU_HEIGHT / 100;
...
motion_draw(uint8_t *i420)
	...
	if (mf->show_vectors)
		{
		i = dav.have_servos ? 0 : 1;
		x = dav.have_servos ? -12 : 0;
	...
	if (!inform_shown && (mf->show_preset || dav.preset_notify))
		{
		for (mrlist = mf->motion_region_list; mrlist; mrlist = mrlist->next)
			...
			if (   dav.on_preset && !dav.servo_moving
		...
		if (dav.on_preset && !dav.preset_notify)
			{
			if (dav.motion_detects_fifo_enable)
	...
	if (vcb->state & VCB_STATE_LOOP_RECORD)
		...
		if (vcb->state & VCB_STATE_MOTION_RECORD)
			msg = dav.external_motion_record_event ?
		...
		{
		if (dav.t_now > dav.motion_sync_time)
			t_record -= dav.t_now - dav.motion_sync_time;
		if (mf->fifo_trigger_time_limit > 0)
			{
			t_hold = vcb->max_record_time -
					(dav.t_now - vcb->record_start_time);
			...
			{
			t_hold = dav.motion_times.event_gap -
					(dav.t_now - dav.motion_last_detect_time);
			if (dav.motion_stills_record)
	...
	if (mf->motion_enable)
		...
		if (vcb->state == VCB_STATE_MANUAL_RECORD)
			msg = "Motion ---";
		else if (dav.servo_moving)
			msg = "Motion hold (moving)";
		else if (!dav.on_preset)
			{
			if (dav.motion_off_preset)
				msg = "Motion ON (off preset)";
			else
				msg = "Motion hold (off preset)";
			}
		else if (dav.motion_stills_enable)
	...
	if (dav.motion_detects_fifo_enable && !*fifo_msg)
	...
	if (time_lapse.show_status)
		...
		if (time_lapse.event)
			t = (int) (time_lapse.event->time - dav.t_now);
	...
	if (dav.video_notify)
		i420_print(&inform_area, normal_font, 0xff, 6, 0, 0, JUSTIFY_CENTER,
					fname_base(dav.video_last));
	if (dav.still_notify)
		i420_print(&inform_area, normal_font, 0xff, 5, 0, 0, JUSTIFY_CENTER,
					fname_base(dav.still_last));
	if (dav.timelapse_notify && time_lapse.show_status)
		i420_print(&inform_area, normal_font, 0xff, 4, 0, 0, JUSTIFY_CENTER,
					fname_base(dav.timelapse_jpeg_last));
	}
...
Adjustment	motion_settings_adjustment[] =
	{
	{ "Startup_Motion",   0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.startup_motion_enable },
	...
	{ "Video_Time_Limit",   0, 1800, 10, 0, 0, 0, 0, "sec", NULL, &dav.motion_record_time_limit },
	{ "Motion_Stills_(no_videos)",   0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.motion_stills_enable },
	{ "Max_Stills_per_Minute", 1, 60, 1, 0, 0, 0, 0, "", NULL, &dav.motion_stills_per_minute}
	};
...
Adjustment	motion_limit_adjustment[] =
	{
	{ "Vector_Magnitude",  2, 50, 1, 0, 0, 0, 0, "", NULL, &dav.motion_magnitude_limit },
	{ "Vector_Count",      2, 50, 1, 0, 0, 0, 0, "", NULL, &dav.motion_magnitude_limit_count },
	{ "Burst_Count",      50, 2000, 10, 0, 0, 0, 0, "", NULL, &dav.motion_burst_count },
	{ "Burst_Frames",      2, 20, 1, 0, 0, 0, 0, "", NULL, &dav.motion_burst_frames },
	{ "Zoom_Percent", 10, 100, 2, 0, 0, 0, 0, "", NULL, &dav.zoom_percent }
	};
...
Adjustment	settings_adjustment[] =
	{
//	{ "Vertical_Filter",  0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.motion_vertical_filter },
	{ "Check_Media_Diskfree",   0, 1,  1, 0, 0, 0, 0, "", NULL, &dav.check_media_diskfree },
	{ "Check_Archive_Diskfree", 0, 1,  1, 0, 0, 0, 0, "", NULL, &dav.check_archive_diskfree },
	{ "Diskfree_Percent",  5,  90, 1, 0, 0, 0, 0, "", NULL, &dav.diskfree_percent },
	...
	{ "mjpeg_divider",  1,  12,   1,   0, 0, 0, 0, "", NULL, &dav.mjpeg_divider },
	{ "still_quality",  5,    100,   1,   0, 0, 0, 0, "", NULL, &camera_adjust_temp.still_quality },
	{ "Vector_Counts",  0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.motion_show_counts },
	{ "Vector_Dimming", 30, 60, 1, 0, 0, 0, 0, "", NULL, &dav.motion_vectors_dimming },
	{ "Preview_Clean",  0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.motion_preview_clean },
	{ "Preview_Stall_Warning",  0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.preview_stall_warning },
	};
...
Adjustment	loop_settings_adjustment[] =
	{
	{ "Startup_Loop",     0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.loop_startup_enable },
	{ "Time_Limit",   10, 900, 10, 0, 0, 0, 0, "sec", NULL, &dav.loop_record_time_limit },
	{ "Diskusage_Percent", 5, 95, 1,   0, 0, 0, 0, "", NULL, &dav.loop_diskusage_percent }
	};
...
Adjustment	audio_settings_adjustment[] =
	{
	{ "Audio_Trigger_Video", 0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.audio_trigger_video },
	{ "Audio_Trigger_Level", 2, 100, 1, 0, 0, 0, 0, "", NULL, &dav.audio_trigger_level },
	{ "Box_MP3_Only",  0, 1, 1, 0, 0, 0, 0, "", NULL, &dav.audio_box_MP3_only }
	};
...
Adjustment	servo_settings_adjustment[] =
	{
	{ "Motion_Off_Preset",   0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.motion_off_preset },
	{ "Move_Step_msec",    0,   200,  1, 0, 0, 0, 0, "", NULL, &dav.servo_move_step_msec },
	{ "Preset_Step_msec",  0,   100,  1, 0, 0, 0, 0, "", NULL, &dav.servo_preset_step_msec },
	{ "Servo_Settle_msec", 200,  1000,  1, 0, 0, 0, 0, "", NULL, &dav.servo_settle_msec },
	{ "Move_Steps",        5,    30,  1, 0, 0, 0, 0, "", NULL, &dav.servo_move_steps },
	{ "Pan_Left_Limit",    SERVO_MIN_WIDTH,  SERVO_MAX_WIDTH,  1, 0, 0, 0, 0, "", NULL, &dav.servo_pan_min },
	{ "Pan_Right_Limit",   SERVO_MIN_WIDTH,  SERVO_MAX_WIDTH,  1, 0, 0, 0, 0, "", NULL, &dav.servo_pan_max },
	{ "Tilt_Up_Limit",     SERVO_MIN_WIDTH,  SERVO_MAX_WIDTH,  1, 0, 0, 0, 0, "", NULL, &dav.servo_tilt_max },
	{ "Tilt_Down_Limit",   SERVO_MIN_WIDTH,  SERVO_MAX_WIDTH,  1, 0, 0, 0, 0, "", NULL, &dav.servo_tilt_min },
	{ "Servo_Pan_Invert",   0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.servo_pan_invert },
	{ "Servo_Tilt_Invert",  0,  1,  1, 0, 0, 0, 0, "", NULL, &dav.servo_tilt_invert }
	};
...
apply_adjustment(void)
	...
	if (adjustments == &motion_settings_adjustment[0])
		{
		dav.motion_times.confirm_gap = motion_times_temp.confirm_gap;
		dav.motion_times.post_capture = motion_times_temp.post_capture;
		if (dav.motion_times.post_capture > motion_times_temp.event_gap)
			motion_times_temp.event_gap = dav.motion_times.post_capture;

		if (   motion_times_temp.pre_capture != dav.motion_times.pre_capture
		    || motion_times_temp.event_gap != dav.motion_times.event_gap
		   )
			{
			pthread_mutex_lock(&vcb->mutex);
			if (vcb->state == VCB_STATE_NONE)
				{
				dav.motion_times.pre_capture = motion_times_temp.pre_capture;
				dav.motion_times.event_gap = motion_times_temp.event_gap;
		...
		if (dav.motion_stills_enable && dav.loop_enable)
			{
			dav.motion_stills_enable = FALSE;
		...
		if (dav.on_preset)
			...
			settings = (PresetSettings *) slist_nth_data(pos->settings_list,
							pos->settings_index);
			settings->mag_limit = dav.motion_magnitude_limit;
			settings->mag_limit_count = dav.motion_magnitude_limit_count;
			settings->burst_count = dav.motion_burst_count;
			settings->burst_frames = dav.motion_burst_frames;
			settings->zoom_percent = dav.zoom_percent;
			dav.preset_modified = TRUE;
			}
		else
			dav.preset_modified_warning = TRUE;
		...
		if (   camera_adjust_temp.video_fps != dav.camera_adjust.video_fps
		    || camera_adjust_temp.still_quality != dav.camera_adjust.still_quality
		    || camera_adjust_temp.video_bitrate != dav.camera_adjust.video_bitrate
		   )
			camera_restart();
		else	/* Pick up changes not requiring new buffer or camera restart */
			dav.camera_adjust = camera_adjust_temp;

		/* All other adjustments have been done live. */
		}
	else if (adjustment_is_servo_limits())
		{
		if (pan_save < dav.servo_pan_min)
			pan_save = dav.servo_pan_min;
		if (pan_save > dav.servo_pan_max)
			pan_save = dav.servo_pan_max;
		if (tilt_save < dav.servo_tilt_min)
			tilt_save = dav.servo_tilt_min;
		if (tilt_save > dav.servo_tilt_max)
			tilt_save = dav.servo_tilt_max;
		servo_move(pan_save, tilt_save, dav.servo_move_step_msec);
		}
...
display_adjustment(uint8_t *i420)
	...
	if (adjustments == &motion_limit_adjustment[0] && !dav.on_preset)
		dav.preset_modified_warning = TRUE;
	else
		dav.preset_modified_warning = FALSE;
	...
	if (cur_adj_start)
		...
		if (adjustments == &picture_adjustment[0])
			...
			if (*display_menu_index == PAN_LEFT_LIMIT_INDEX)
				{
				cur_adj->value = dav.servo_pan_min;
				servo_move(dav.servo_pan_min, tilt_save, dav.servo_move_step_msec);
				}
			else if (*display_menu_index == PAN_RIGHT_LIMIT_INDEX)
				{
				cur_adj->value = dav.servo_pan_max;
				servo_move(dav.servo_pan_max, tilt_save, dav.servo_move_step_msec);
				}
			else if (*display_menu_index == TILT_UP_LIMIT_INDEX)
				{
				cur_adj->value = dav.servo_tilt_max;
				servo_move(pan_save, dav.servo_tilt_max, dav.servo_move_step_msec);
				}
			else if (*display_menu_index == TILT_DOWN_LIMIT_INDEX)
				{
				cur_adj->value = dav.servo_tilt_min;
				servo_move(pan_save, dav.servo_tilt_min, dav.servo_move_step_msec);
				}
	...
	if (cur_adj->value != cur_adj->prev_value)
		...
		if (adjustment_is_zoom())
			...
			if (cur_adj->value <= min || cur_adj->value >= max)
				...
				servo_move(cur_adj->value, tilt, dav.servo_move_step_msec);
				}
			else
				{
...
//max, min, cur_adj->value, cur_adj->prev_value, tilt);
			...
			if (cur_adj->value <= min || cur_adj->value >= max)
				{
				servo_get_position(&pan, &tilt);
				servo_move(pan, cur_adj->value, dav.servo_move_step_msec);
				}
			else
				{
				cur_adj->value = cur_adj->prev_value;
...
//	printf("%s: %d\n", dcmd->name, dcmd->action);
	...
	switch (dcmd->action)
		...
		case SELECT:
			if (display_state == DISPLAY_ADJUSTMENT)
				{
				dav.config_modified = apply_adjustment();
				...
				if (   width != dav.camera_config.video_width
				    || height != dav.camera_config.video_height
				   )
					{
					dav.camera_config.video_width = width;
					dav.camera_config.video_height = height;
					dav.config_modified = TRUE;
					camera_restart();
					}
				display_state = DISPLAY_MENU;
		    	...
		    	entry = (MenuEntry *) slist_nth_data(display_menu_list,
										*display_menu_index);
				sscanf(entry->name, "%dx%d", &width, &height);
				if (   width != dav.camera_config.still_width
				    || height != dav.camera_config.still_height
				   )
					{
					dav.camera_config.still_width = width;
					dav.camera_config.still_height = height;
					dav.config_modified = TRUE;
					camera_restart();
					}
				display_state = DISPLAY_MENU;
		...
		case VIDEO_PRESET:
			...
			for (i = N_VIDEO_PRESET_ENTRIES - 1; i > 0 ; --i)
				...
				if (   dav.camera_config.video_width == width
				    && dav.camera_config.video_height == height
				   )
					break;
				}
		...
		case STILL_PRESET:
			...
			for (i = 0, list = display_menu_list; list; ++i, list = list->next)
				...
				entry = (MenuEntry *) list->data;
				sscanf(entry->name, "%dx%d", &width, &height);
				if (   dav.camera_config.still_width == width
				    && dav.camera_config.still_height == height
				   )
					break;
				}
...
display_inform(char *args)
	...
	if (sscanf(args, "timeout %d\n", &n) == 1)
		{
		n = (n < 0 || n > 30) ? dav.notify_duration : n;
		if (n == 0)
			{
...
display_draw(uint8_t *i420)
	...
	|  do not draw.
	*/
	if (dav.do_preview_save && dav.motion_preview_clean)
	...
	*/
	...
	if (!menu_still_presets_list)
		...
		for (i = 0, position = 0; i < N_STILL_PRESET_ENTRIES; ++i)
			...
			if (   dav.camera_width_max >= width
			    && dav.camera_height_max >= height
			   )
				{
				entry = calloc(1, sizeof(MenuEntry));
