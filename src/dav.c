...
substitute_var(char *str, char V, char *fmt_arg)
  ...
  |  conversions and up to two dav filename substitution variables.
...
log_printf_no_timestamp(char *fmt, ...)
	...
	va_start(args, fmt);
	if (dav.verbose || dav.verbose_log)
		vfprintf(stderr, fmt, args);
	if ((f = fopen(dav.log_file, "a")) != NULL)
		{
		vfprintf(f, fmt, args);
		fclose(f);
		}
...
log_printf(char *fmt, ...)
	...
	va_start(args, fmt);
	strftime(tbuf, sizeof(tbuf), "%T", localtime(&dav.t_now));
	if (dav.verbose || dav.verbose_log)
		{
...
annotate_string_remove(char *id)
	...
	for (list = dav.annotate_list; list; list = list->next)
		...
		if (!strcmp(annotate->id, id))
			{
			dav.annotate_list = slist_remove(dav.annotate_list, annotate);
			free(annotate->id);
...
camera_start(void)
	...
	if (!camera_create())
		...
		fprintf(stderr, "See /tmp/dav.log for an error message.\n");
		asprintf(&cmd, "cp %s/www/images/camera-error.jpg %s",
				dav.install_dir, dav.mjpeg_filename);
		exec_wait(cmd, NULL);
		exit(1);
		}
	...
	*/
	dav.mjpeg_height = dav.mjpeg_width *
					dav.camera_config.video_height / dav.camera_config.video_width;
	dav.mjpeg_width &= ~0xf;		/* Make resize multiple of 16 */
	dav.mjpeg_height &= ~0xf;
	resizer_create("stream_resizer", &stream_resizer,
							camera.component->output[CAMERA_PREVIEW_PORT],
							dav.mjpeg_width, dav.mjpeg_height);
	ports_tunnel_connect(&camera, CAMERA_PREVIEW_PORT, &stream_resizer);
	jpeg_encoder_create("mjpeg_encoder", &mjpeg_encoder,
					stream_resizer.component->output[0], dav.mjpeg_quality);
	...
	*/
	jpeg_encoder_create("still_jpeg_encoder", &still_jpeg_encoder,
					NULL, dav.camera_adjust.still_quality);
...
camera_restart(void)
	...
	dav.camera_adjust = camera_adjust_temp;	/* May not be changed */
	...
	*/
	...
	if ((still_jpeg_encoder.file = fopen(fname, "w")) == NULL)
		...
		if ((status = mmal_port_parameter_set_boolean(
			...
			dup_string(&dav.still_last, fname);
			n = dav.notify_duration * EVENT_LOOP_FREQUENCY;
			...
			if (!event && !motion_still)
				event_count_down_add("still saved", n,
						event_notify_expire, &dav.still_notify);
			dav.still_capture_event = TRUE;
			dav.motion_still_capture_event = motion_still;
			if (!motion_still)
				dav.still_notify = TRUE;
			}
		}
	return result;
	}
...
timelapse_capture(void)
	...
	*/
	...
	path = media_pathname(dav.timelapse_dir,
						dav.timelapse_format, 0,
						'N',  seq_buf,
						'n',  series_buf);
	...
	if ((still_jpeg_encoder.file = fopen(path, "w")) == NULL)
		...
		{
		dup_string(&dav.timelapse_jpeg_last, path);
		dav.timelapse_capture_event = TRUE;
		if ((status = mmal_port_parameter_set_boolean(
						camera.component->output[CAMERA_CAPTURE_PORT],
						MMAL_PARAMETER_CAPTURE, 1)) != MMAL_SUCCESS)
			{
			...
			*/
			...
			event_count_down_add("timelapse saved", nd,
					event_notify_expire, &dav.timelapse_notify);
			dav.timelapse_notify = TRUE;
			}
		}
	free(path);
	}
...
timelapse_inform_convert(void)
	...
	snprintf(fname, sizeof(fname), "%s/%s.mp4",
			dav.timelapse_dir, time_lapse.convert_name);
	st.st_size = 0;
	stat(fname, &st);
...
video_record_start(VideoCircularBuffer *vcb, int start_state)
	...
	if (start_state == VCB_STATE_MOTION_RECORD_START)
		...
		path = media_pathname(dav.video_dir,
					dav.video_motion_name_format,
					vcb->key_frame[n].t_frame,
					'N',  seq_buf, 'H', dav.hostname);
		dav.video_motion_sequence += 1;
		dav.config_media_sequence_modified = TRUE;
		if (dav.motion_stats)
			do_stats = TRUE;
		dav.video_description = "Motion";
		}
	else if (start_state == VCB_STATE_LOOP_RECORD_START)
		{
		n = (dav.loop_next_keyframe >= 0)
				? dav.loop_next_keyframe : vcb->cur_frame_index;
		path = media_pathname(dav.loop_video_dir,
						dav.loop_name_format,
//						vcb->key_frame[n].t_frame,
						vcb->loop_start_time,
						'N',  "0", 'H', dav.hostname);
		dav.video_description = "Loop";
		}
		...
		path = media_pathname(dav.video_dir,
						dav.video_manual_name_format,
						vcb->key_frame[n].t_frame,
						'N',  seq_buf, 'H', dav.hostname);
		dav.video_manual_sequence += 1;
		dav.config_media_sequence_modified =  TRUE;
		dav.video_description = "Manual";
		}
	...
	dav.video_start_pts = vcb->key_frame[n].frame_pts;

	if (dav.audio_pathname)
		free(dav.audio_pathname);
	dav.audio_pathname = NULL;

	dav.thumb_convert_done = FALSE;

	dup_string(&dav.video_pathname, path);
	free(path);
	make_preview_pathname(dav.video_pathname);
	path = dav.video_pathname;
	...
	if ((s = strstr(path, ".mp4")) != NULL && *(s + 4) == '\0')
		...
		*s = '\0';	/* temporarily truncate .mp4 from video_pathname */
		...
		if (dav.audio_enable)
			...
			v = strrchr(path, '/');
			if (v)
				asprintf(&dav.audio_pathname, "%s/%s.mp3",
							dav.tmpfs_dir, v + 1);
			else
				asprintf(&dav.audio_pathname, "%s.mp3", path);
		...
		if (dav.video_h264)
			free(dav.video_h264);
		asprintf(&dav.video_h264, "%s.h264", path);
		*s = '.';		/* restore .mp4 to video_pathname */
		path = dav.video_h264;
		dav.video_mp4box = TRUE;
		}
	else
		dav.video_mp4box = FALSE;
	...
	if ((vcb->file = fopen(path, "w")) == NULL)
		...
		dav.state_modified = TRUE;
		dav.do_preview_save = TRUE;
		dav.do_thumb_convert = TRUE;
		...
		if (start_state == VCB_STATE_MOTION_RECORD_START)
			...
			motion_events_write(&motion_frame, MOTION_EVENTS_START,
					...
					/ (float) dav.camera_adjust.video_fps);
			if (*dav.on_motion_begin_cmd != '\0')
				event_add("motion begin", dav.t_now, 0,
					event_motion_begin_cmd, dav.on_motion_begin_cmd);
			}
		log_printf("%s record start - %s\n", dav.video_description, path);
		}
	if (stats_path)
		free(stats_path);
	}
...
thumb_convert(void)
	...
	base = fname_base(dav.preview_pathname);
	if ((s = strstr(base, ".jpg")) != NULL)
		{
		*s = '\0';
		snprintf(buf, sizeof(buf), "%s.th.jpg", base);
		dup_string(&dav.thumb_name, buf);
		*s = '.';
		}

	if (   (vcb->state & VCB_STATE_MANUAL_RECORD)
	    || (vcb->state & VCB_STATE_LOOP_RECORD)
	    || (   dav.external_motion_record_event
	        && !dav.motion_stills_record
	       )
	   )
		h = 150 * dav.camera_config.video_height
				/ dav.camera_config.video_width;
	else if  (dav.motion_stills_record)
		h = 150 * dav.camera_config.still_height
	...
	if (   (   (vcb->state & VCB_STATE_MOTION)
		...
		if (   (vcb->state & VCB_STATE_LOOP_RECORD)
		    && (s = strstr(dav.thumb_name, "0.th.jpg")) != NULL
		   )
			*s = 'm';
		}
	else
	...
	thumb_cmd = expand_command(buf, dav.preview_pathname);
	...
	*/
	if (   *dav.on_motion_preview_save_cmd
	    && (   (vcb->state & VCB_STATE_MOTION_RECORD)
	        || (   dav.motion_stills_record
			    && !dav.thumb_convert_done
	           )
	       )
	   )
		{
		preview_cmd = expand_command(dav.on_motion_preview_save_cmd,
				dav.preview_pathname);
		asprintf(&cmd, "%s; %s; rm -f %s",
	         	thumb_cmd, preview_cmd, dav.preview_pathname);
		free(preview_cmd);
		}
	else
		asprintf(&cmd, "%s; rm -f %s", thumb_cmd, dav.preview_pathname);
	free(thumb_cmd);

	dav.thumb_convert_done = TRUE;
...
video_record_stop(VideoCircularBuffer *vcb)
	...
	*/
	dav.video_last_frame_count = vcb->video_frame_count;
	dav.video_end_pts = vcb->last_pts;
	if (dav.video_last_frame_count > 1)
		{
		ftmp = (double)(dav.video_end_pts - dav.video_start_pts) / 1e6;
		ftmp /= dav.video_last_frame_count - 1;
		}
	else
		ftmp = 0;
	ftmp *= dav.video_last_frame_count;
	dav.video_last_time = ftmp;
	dav.video_last_fps = (ftmp > 0) ?
			(double) dav.video_last_frame_count / ftmp : 0;
	...
	*/
	if (vcb->state & VCB_STATE_LOOP_RECORD)
		{
		s = strstr(dav.video_pathname, "0.mp4");
		if (s && dav.external_motion_record_event)
			{
			if (mf->fifo_detects)
				{
				*s = '\0';
				asprintf(&mod_name, "%se-%s.mp4", dav.video_pathname,
							mf->fifo_trigger_code);
				free(dav.video_pathname);
				dav.video_pathname = mod_name;
				}
			else
	...
	else if (   dav.external_motion_record_event
	         && (s = strstr(dav.video_pathname, "motion_")) != NULL
	        )
		{
		*s = '\0';
		if (mf->fifo_detects)
			asprintf(&mod_name, "%sext-%s%s", dav.video_pathname,
					mf->fifo_trigger_code ? mf->fifo_trigger_code : "FIFO",
					s + 6);
		else
			asprintf(&mod_name, "%sAudio%s", dav.video_pathname, s + 6);
	...
	dup_string(&dav.video_last, dav.video_pathname);
	
	if (video_name_modified)
		{
		base = fname_base(dav.video_pathname);
		if (   (s = strstr(base, ".mp4")) != NULL
			...
			*s = '\0';
			thumb_dir = (vcb->state & VCB_STATE_LOOP_RECORD) ?
					dav.loop_thumb_dir : dav.thumb_dir;
			...
			dup_string(&dav.thumb_name, buf2);
			*s = '.';
			}
		}

	if (   (vcb->state & VCB_STATE_MOTION_RECORD)
	    && *dav.on_motion_end_cmd
	   )
		record_end_cmd = expand_command(dav.on_motion_end_cmd, NULL);
	else if (   (vcb->state & VCB_STATE_MANUAL_RECORD)
	         && *dav.on_manual_end_cmd
		    )
		record_end_cmd = expand_command(dav.on_manual_end_cmd, NULL);
	else if (   (vcb->state & VCB_STATE_LOOP_RECORD)
		     && *dav.on_loop_end_cmd
		    )
		record_end_cmd = expand_command(dav.on_loop_end_cmd, NULL);
	...
	if (st_h264.st_size > 0)  // can be 0 if no space left
		...
		if (dav.video_mp4box)
			{
			get_tmp_space_avail();
			tmp_space = (dav.tmp_space_avail / 4) * 3;
			tmp_dir = ((uint64_t) st_h264.st_size < tmp_space)
						? "/tmp" : dav.video_dir;

			if (dav.camera_adjust.video_mp4box_fps == 0)
				encode_fps = dav.video_last_fps;
			else
				encode_fps = (double) dav.camera_adjust.video_mp4box_fps;

			converting_file = (vcb->state & VCB_STATE_LOOP_RECORD)
				? dav.loop_converting : dav.video_converting;

			if (dav.audio_pathname)
				asprintf(&add_mp3, "-add %s", dav.audio_pathname);

			if (   (vcb->state & VCB_STATE_MANUAL_RECORD)
			    || (vcb->state & VCB_STATE_LOOP_RECORD)
				|| !(   dav.audio_box_MP3_only
			         && dav.external_motion_record_event
			         && mf->fifo_detects == 0
			        )
			   )
				asprintf(&add_h264, "-fps %.3f -add %s",
						(float) encode_fps, dav.video_h264);
...
//					dav.video_h264,
					...
					add_mp3 ? add_mp3 : "",
					dav.video_pathname,
					dav.verbose ? "" : "2> /dev/null",

					dav.video_h264,			// rm h264 mp3
					dav.audio_pathname ? dav.audio_pathname : "",
					converting_file,			// rm
			...
			exec_no_wait(record_end_cmd, NULL, dav.verbose_log);
		}
	else
		asprintf(&cmd, "rm -f %s", dav.video_h264);
	...
	log_printf("%s record stop (size:%.1fMB %s /tmp:%.1fMB)  vid_time:%.2f  vid_fps:%.2f  audio_frames:%d  audio_rate:%d\n",
			dav.video_description,
			(float) st_h264.st_size / 1e6,
			strcmp(tmp_dir, "/tmp") ? ">" : "<",
			(float) (tmp_space / 1e6),
			(float) dav.video_last_time,
			(float) dav.video_last_fps,
			dav.audio_last_frame_count,
			dav.audio_last_rate);
...
"    first detect: %s  totals - direction:%d  burst:%d  max burst count:%d  audio:%d  external:%d\n",
	...
	dav.video_notify = TRUE;
	event_count_down_add("video saved notify",
				dav.notify_duration * EVENT_LOOP_FREQUENCY,
				event_notify_expire, &dav.video_notify);

	if (vcb->state & VCB_STATE_LOOP_RECORD)
		event_add("loop diskusage percent", dav.t_now, 0,
					event_loop_diskusage_percent, NULL);
	else if (dav.check_media_diskfree)
		event_add("media diskfree percent", dav.t_now, 0,
	...
	mf->burst_detects = 0;
	mf->fifo_detects = 0;
	dav.external_motion_record_event = FALSE;
...
loop_record(void)
	...
	if (dav.loop_enable && vcb->state == VCB_STATE_NONE)
		{
		pthread_mutex_lock(&vcb->mutex);
		if (   t_loop_start == 0
		    || last_time_limit != dav.loop_record_time_limit
		   )
			{
		...
		vcb->loop_start_time =
					t_loop_start + n_loops * dav.loop_record_time_limit;
		++n_loops;
		n = (dav.loop_next_keyframe >= 0)
				? dav.loop_next_keyframe : vcb->cur_frame_index;
		vcb->max_record_time = n_loops * dav.loop_record_time_limit
	...
	else if (!dav.loop_enable)
		{
		t_loop_start = 0;
		n_loops = 0;
		dav.loop_next_keyframe = -1;
		}
	}
...
get_arg_pass1(char *opt, char *arg)
	...
	if (!strcmp(opt, "-v"))
		...
		dav.verbose_motion = TRUE;
	else if (!strcmp(opt, "-vmulti"))
		dav.verbose_multicast = TRUE;
...
dav_cleanup(void)
	...
	preset_state_save();
	if (dav.config_modified)
		config_save(dav.config_file);
	if (dav.preset_modified)
		preset_config_save();
	...
	if (dav.audio_pathname)
		unlink(dav.audio_pathname);
	if (dav.video_h264)
		unlink(dav.video_h264);
	unlink(dav.loop_converting);
	unlink(dav.video_converting);
	if (dav.preview_pathname)
		unlink(dav.preview_pathname);
	vcb->state = VCB_STATE_NONE;
	dav.loop_enable = FALSE;
...
command_process(char *command_line)
	...
	switch (cmd->code)
		...
		case still:
			snprintf(buf, sizeof(buf), "%d", dav.still_sequence);
			path = media_pathname(dav.still_dir,
							dav.still_name_format, 0,
							'N', buf,
							'H', dav.hostname);
			dav.still_sequence += 1;
			dav.config_media_sequence_modified =  TRUE;
		...
		case tl_start:
			...
			if (!time_lapse.event)
				...
				time_lapse.event = event_add("timelapse",
							dav.t_now, n, timelapse_capture, NULL);
		...
		case tl_end:
			if (time_lapse.activated)
				...
				dav.state_modified = TRUE;
				config_timelapse_save_status();
				exec_no_wait(dav.timelapse_convert_cmd, NULL, TRUE);
		...
		case tl_inform_convert:
			if (sscanf(args, "%127s %63[^\n]", arg1, arg2) == 2)
				...
				if (!strcmp(arg1, "done"))
					...
					dup_string(&dav.timelapse_video_last, dav.timelapse_video_pending);
					dup_string(&dav.timelapse_video_pending, "");
					dup_string(&dav.timelapse_jpeg_last, "");
					dav.state_modified = TRUE;
					...
					time_lapse.inform_event = event_add("tl_inform_convert",
							dav.t_now, 5, timelapse_inform_convert, NULL);
					}
				}
			break;
		...
		case motion_enable:
			...
			if (   n != motion_frame.motion_enable
			    && *dav.on_motion_enable_cmd
			   )
				event_add("motion enable", dav.t_now, 0,
					event_motion_enable_cmd, dav.on_motion_enable_cmd);
			break;

		case motion_detects_fifo_enable:
			config_set_boolean(&dav.motion_detects_fifo_enable, args);
		...
		case video_bitrate:
			...
			dav.camera_adjust.video_bitrate = n;
			camera_restart();
			dav.config_modified = TRUE;
			break;
		...
		case still_quality:
			...
			dav.camera_adjust.still_quality = n;
			camera_restart();
			dav.config_modified = TRUE;
			break;
		...
		case video_mp4box_fps:
			...
			dav.camera_adjust.video_mp4box_fps = f;
			dav.config_modified = TRUE;
			break;
		...
		case archive_video:		/* ["day"|video.mp4] yyyy-mm-dd */
			if (sscanf(args, "%127s %63s", arg1, arg2) == 2)
				...
				snprintf(buf, sizeof(buf), fmt,
						dav.install_dir, arg1, arg2);
				exec_no_wait(buf, NULL, TRUE);
				if (dav.check_archive_diskfree)
					event_add("archive diskfree percent", dav.t_now + 3,
		...
		case annotate_string:		/* annotate_string cmd id text */
			...
			if (n == 2 && !strcmp(arg1, "remove"))
				...
				dav.annotate_string_space_char = arg2[0];
			else if (n == 3)
		...
		case verbose:
			config_set_boolean(&dav.verbose, args);
			break;

		case verbose_log:
			config_set_boolean(&dav.verbose_log, args);
			break;
		...
		case halt:
			if (dav.halt_enable)
				...
				display_inform("\"Set halt_enable in dav.conf first.\" 4 3 1");
				display_inform("timeout 3");
				}
			break;
...
log_start(boolean start_sep, boolean time, boolean end_sep)
	...
	if (time)
		{
		strftime(buf, sizeof(buf), "%F %T", localtime(&dav.t_now));
		log_printf_no_timestamp("======= dav %s started at %s\n",
					dav.version, buf);
		}
	if (end_sep)
		log_printf_no_timestamp("========================================================\n");
...
//	struct timeval	tv;
	...
	if (!config_load(dav.config_file))
		config_save(dav.config_file);
	if (quit_flag)	/* Just making sure initial config file is written */
		exit(0);

	if (!dav.log_file || !*dav.log_file)
		dav.log_file = strdup("/dev/null");
	else if (*dav.log_file != '/')
		{
		snprintf(buf, sizeof(buf), "%s/%s", dav.install_dir, dav.log_file);
		dup_string(&dav.log_file, buf);
		}

	if (*dav.lc_time)
		{
		log_printf("setting LC_TIME to %s\n", dav.lc_time);
		setlocale(LC_TIME, dav.lc_time);
		}
	dav.pi_model = pi_model();

	/* If need to mmap() gpios for servos, restart a sudo dav which can
	...
	*/
	...
	else if (dav.have_servos && !dav.servo_use_servoblaster)
		{
		/* Need to restart dav as root so can mmap() PWMs for servos.
		*/
	...
	else if (dav.servo_use_servoblaster)
		{
	...
	dav.effective_user = strdup(user ? user : "pi");

	if (!motion_regions_config_load(dav.motion_regions_config_file, FALSE))
		motion_regions_config_save(dav.motion_regions_config_file, FALSE);
	preset_config_load();

	if (!at_commands_config_load(dav.at_commands_config_file))
		at_commands_config_save(dav.at_commands_config_file);
	...
	if (*dav.media_dir != '/')
		{
		snprintf(buf, sizeof(buf), "%s/%s", dav.install_dir, dav.media_dir);
		dup_string(&dav.media_dir, buf);
		}
	if (*dav.archive_dir != '/')
		{
		snprintf(buf, sizeof(buf), "%s/%s", dav.media_dir, dav.archive_dir);
		dup_string(&dav.archive_dir, buf);
		}
	if (*dav.loop_dir != '/')
		{
		snprintf(buf, sizeof(buf), "%s/%s", dav.media_dir, dav.loop_dir);
		dup_string(&dav.loop_dir, buf);
		}

	snprintf(buf, sizeof(buf), "%s/%s", dav.install_dir, "www");
	check_modes(buf, 0775);

	asprintf(&dav.command_fifo, "%s/www/FIFO", dav.install_dir);
	asprintf(&dav.motion_detects_fifo, "%s/www/motion_detects_FIFO", dav.install_dir);
	asprintf(&dav.audio_fifo, "%s/www/audio_FIFO", dav.install_dir);
	asprintf(&dav.scripts_dir, "%s/scripts", dav.install_dir);
	asprintf(&dav.scripts_dist_dir, "%s/scripts-dist", dav.install_dir);
	asprintf(&dav.mjpeg_filename, "%s/mjpeg.jpg", dav.tmpfs_dir);
	asprintf(&dav.state_filename, "%s/state", dav.tmpfs_dir);

	asprintf(&dav.video_converting, "%s/video_converting", dav.tmpfs_dir);
	asprintf(&dav.loop_converting, "%s/loop_converting", dav.tmpfs_dir);

	log_printf_no_timestamp("command FIFO: %s\n", dav.command_fifo);
	log_printf_no_timestamp("motion_detects FIFO  : %s\n", dav.motion_detects_fifo);
	...
	*/
	asprintf(&dav.video_dir, "%s/%s", dav.media_dir, dav_VIDEO_SUBDIR);
	asprintf(&dav.thumb_dir, "%s/%s", dav.media_dir, dav_THUMBS_SUBDIR);
	asprintf(&dav.loop_video_dir, "%s/%s", dav.loop_dir, dav_VIDEO_SUBDIR);
	asprintf(&dav.loop_thumb_dir, "%s/%s", dav.loop_dir, dav_THUMBS_SUBDIR);

	asprintf(&dav.still_dir, "%s/%s", dav.media_dir, dav_STILL_SUBDIR);
	asprintf(&dav.still_thumb_dir, "%s/%s/.thumbs", dav.media_dir, dav_STILL_SUBDIR);

	asprintf(&dav.timelapse_dir, "%s/%s", dav.media_dir, dav_TIMELAPSE_SUBDIR);

	/* Make directories in case startup script needs them, but dont't error
	|  out in case startup script will do something about it.
	*/
	if (   !make_dir(dav.media_dir)		// after startup script, will make
		|| !make_dir(dav.archive_dir)	// dirs again in case of mount
		|| !make_dir(dav.loop_dir)
	   )
		{
		log_printf_no_timestamp(
	...
	snprintf(buf, sizeof(buf), "%s/scripts-dist/_init $I $a $m $M $P $G %s %s",
		dav.install_dir,
		(dav.have_servos) ? "servos_on" : "servos_off",
		dav.loop_dir);
	exec_wait(buf, NULL);

	/* User may have enabled a mount disk on media_dir
	*/
	exec_wait(dav.on_startup_cmd, NULL);
	check_modes(dav.media_dir, 0775);
	check_modes(dav.archive_dir, 0775);
	check_modes(dav.loop_dir, 0775);
	check_modes(dav.log_file, 0664);

	if (   !make_dir(dav.tmpfs_dir)
	    || !make_dir(dav.video_dir)
	    || !make_dir(dav.thumb_dir)
		|| !make_dir(dav.loop_dir)
	    || !make_dir(dav.still_dir)
	    || !make_dir(dav.timelapse_dir)
	    || !make_fifo(dav.command_fifo)
	   )
		{
		log_printf_no_timestamp(
	...
	if (dav.audio_enable)
		...
		if (!audio_mic_open(FALSE))
			event_add("audio retry open", dav.t_now, 5,
					audio_retry_open, NULL);
		}
	dav.loop_enable = dav.loop_startup_enable;
	...
	while (1)
		{
		if (gettimeofday(&dav.tv_now, NULL) < 0)
			{
			...
			t_usleep = (int) (100000 - (dav.tv_now.tv_usec % 100000));
			usleep(t_usleep + 1);
			}
		time(&dav.t_now);
