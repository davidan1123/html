...
//		tilt, tilt_cur, tilt_delta, tilt_inc);
	...
	for (i = 1; i < max_delta && delay > 0; ++i)
		...
		pan_cur += pan_inc;
		tilt_cur += tilt_inc;
		pwm_width_func(pan_channel, (int) pan_cur, dav.servo_pan_invert);
		pwm_width_func(tilt_channel, (int) tilt_cur, dav.servo_tilt_invert);
		usleep(delay * 1000);
		...
		if (servo_control.status != SERVO_MOVING)
			...
			if (servo_control.status != SERVO_NEW_MOVE)
				...
				usleep(dav.servo_settle_msec * 1000);
				dav.servo_moving = FALSE;
				dav.state_modified = TRUE;
				dav.preset_state_modified = TRUE;
				}
			else
				pthread_mutex_unlock(&servo_control.mutex);
			return;
	...
	pan_cur = (float) pan;
	tilt_cur = (float) tilt;
	pwm_width_func(pan_channel, pan, dav.servo_pan_invert);
	pwm_width_func(tilt_channel, tilt, dav.servo_tilt_invert);
	pthread_mutex_lock(&servo_control.mutex);
	if (servo_control.status != SERVO_NEW_MOVE)
		{
...
servo_thread(void *ptr)
	...
	while (1)
		...
		if (!first_move_done)	/* Minimize possible current spike */
			...
			dav.servo_moving = TRUE;
			pthread_mutex_lock(&servo_control.mutex);
			pwm_width_func(pan_channel, servo_control.pan, dav.servo_pan_invert);
			usleep(300000);
			pwm_width_func(tilt_channel, servo_control.tilt, dav.servo_tilt_invert);
			usleep(300000);
...
servo_init(void)
	...
	log_printf_no_timestamp(
				...
				dav.pi_model, peripheral_base);

	if (dav.servo_use_servoblaster)
		{
		pwm_width_func = pwm_width_servoblaster;
		pan_channel = dav.servo_pan_gpio;
		tilt_channel = dav.servo_tilt_gpio;
	...
	gpio_to_channel(dav.servo_pan_gpio, &pan_channel, &pan_alt);
	gpio_to_channel(dav.servo_tilt_gpio, &tilt_channel, &tilt_alt);
	if (   (pan_channel < 0 && tilt_channel < 0)
	    || (pan_channel == tilt_channel)
	   )
		{
	...
	if (pan_channel > 0)
		gpio_alt_function(dav.servo_pan_gpio, pan_alt);
	if (tilt_channel > 0)
		gpio_alt_function(dav.servo_tilt_gpio, tilt_alt);
	...
	*/
	...
	if (dav.servo_pan_invert)
		pan_cur = 300.0 - pan_cur;
	if (pan_cur < dav.servo_pan_min || pan_cur > dav.servo_pan_max)
		pan_cur = 150;

	tilt_cur = (float) ((tilt_channel == 1) ? t1 : t2);
	if (dav.servo_tilt_invert)
		tilt_cur = 300.0 - tilt_cur;
	if (tilt_cur < dav.servo_tilt_min || tilt_cur > dav.servo_tilt_max)
		tilt_cur = 150;
	log_printf_no_timestamp("======= Servo using hardware PWM (%d %d)\n",
					dav.servo_pan_gpio, dav.servo_tilt_gpio);
...
//      inform_message("Bad motion command.");
	...
	switch (id)
		{
		case PAN_LEFT:
			pan = servo_move_position(pan, -1, mode, dav.servo_pan_min);
			if (pan < dav.servo_pan_min)
				pan = dav.servo_pan_min;
			servo_move(pan, tilt, dav.servo_move_step_msec);
			break;

		case PAN_RIGHT:
			pan = servo_move_position(pan, 1, mode, dav.servo_pan_max);
			if (pan > dav.servo_pan_max)
				pan = dav.servo_pan_max;
			servo_move(pan, tilt, dav.servo_move_step_msec);
			break;

		case TILT_UP:
			tilt = servo_move_position(tilt, 1, mode, dav.servo_tilt_max);
			if (tilt > dav.servo_tilt_max)
				tilt = dav.servo_tilt_max;
			servo_move(pan, tilt, dav.servo_move_step_msec);
			break;

		case TILT_DOWN:
			tilt = servo_move_position(tilt, -1, mode, dav.servo_tilt_min);
			if (tilt < dav.servo_tilt_min)
				tilt = dav.servo_tilt_min;
			servo_move(pan, tilt, dav.servo_move_step_msec);
			break;
		}
	}
