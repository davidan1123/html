...
motion_stats_write(VideoCircularBuffer *vcb, MotionFrame *mf)
	...
	fprintf(vcb->motion_stats_file,
		...
		(float) vcb->frame_count / (float) dav.camera_adjust.video_fps,
...
motion_events_write(MotionFrame *mf, int type, float detect_time)
	...
	if (type & MOTION_EVENTS_HEADER)
		{
		f = fopen(dav.motion_events_filename, "w");
		fprintf(f, "<header>\n");
		fprintf(f, "video %s\n", dav.video_pathname);
		fprintf(f, "frame %d %d\n", mf->width, mf->height);
...
motion_detects_fifo_write(MotionFrame *mf)
	...
	if (!mf)
		{
		if (!dav.motion_detects_fifo_enable && fd >= 0)
			{
			write(fd, "<off>\n", 6);
			close(fd);
			fd = -1;
			}
		else if (dav.motion_detects_fifo_enable && fd < 0)
			fd = open(dav.motion_detects_fifo, O_WRONLY | O_NONBLOCK);
	...
	i = snprintf(buf, sizeof(buf), "<motion %d.%d >\n",
			(int) dav.tv_now.tv_sec,
			(int) dav.tv_now.tv_usec / 100000);
	write(fd, buf, i);
...
motion_stills_pathname(MotionFrame *mf, time_t t)
	...
	snprintf(buf, sizeof(buf), "%d", dav.motion_stills_sequence);

	if (mf->motion_status & (MOTION_DIRECTION | MOTION_BURST))
		asprintf(&name_format, "motion_%s",
		         dav.motion_stills_name_format);
	else
		{
		if (mf->motion_status & MOTION_FIFO)
	...
	path = media_pathname(dav.still_dir, name_format, t,
...
motion_stills_start(MotionFrame *mf)
	...
	if (mf->motion_status & (MOTION_DIRECTION | MOTION_BURST))
		dav.external_motion_still = FALSE;
	else
		dav.external_motion_still = TRUE;

	dav.do_preview_save = TRUE;
	dav.do_thumb_convert = TRUE;
	dav.thumb_convert_done = FALSE;
	dav.do_motion_still = TRUE;
	dav.motion_still_pathname = path;
...
motion_stills_stop(void)
	...
	dav.motion_stills_record = FALSE;
	dav.external_motion_still = FALSE;

	motion_events_write(&motion_frame, MOTION_EVENTS_END, 0);

	if (*dav.on_motion_end_cmd)
		{
		motion_end_cmd = expand_command(dav.on_motion_end_cmd, NULL);
...
motion_frame_process(VideoCircularBuffer *vcb, MotionFrame *mf)
	...
	*/
	...
	mf->mag2_limit  = dav.motion_magnitude_limit * dav.motion_magnitude_limit;
	mf->mag2_limit_count = dav.motion_magnitude_limit_count;
	...
	*/
	if (   burst_density_pass
	    && frame_vec->mag2_count + mf->reject_count >
				dav.motion_burst_count + (int) mf->any_count_expma
	   )
		{
		if (motion_burst_frame < dav.motion_burst_frames)
			++motion_burst_frame;
	...
	if (   motion_count > 0
		...
		if (   !(vcb->state & VCB_STATE_MOTION_RECORD)
		    ...
		    && mf->frame_window == 0
		    && dav.motion_times.confirm_gap > 0
		   )
			{
			mf->frame_window = dav.camera_adjust.video_fps *
					dav.motion_times.confirm_gap / dav.mjpeg_divider;
	...
	dav.audio_level_event = dav.audio_level * 100 / INT16_MAX;
	if (   dav.audio_trigger_video
	    && dav.audio_level_event >= dav.audio_trigger_level
	   )
		{
		mf->motion_status &= ~(MOTION_PENDING_DIR | MOTION_PENDING_BURST);
...
"frame:   x,y(%d,%d) dx,dy(%d,%d) mag2:%d count,rej(%d,%d) any[%d,%.1f] burst:%d\n",
	...
	if (dav.verbose_motion && (fail_count > 0 || motion_count > 0))
		{
		strftime(tbuf, sizeof(tbuf), "%T", &dav.tm_local);
		msg = "";
		if ((mf->motion_status & (MOTION_DIRECTION | MOTION_BURST))
	...
	motion_enabled = (  (  mf->motion_enable
	                  ...
	                  && !dav.servo_moving
					  && dav.zoom_motion_holdoff == 0
					  && (dav.on_preset || dav.motion_off_preset)
	                 );
	if (dav.zoom_motion_holdoff > 0)
		--dav.zoom_motion_holdoff;
	...
	if (   (   (mf->motion_status & MOTION_DETECTED)
		...
		{
		dav.motion_last_detect_time = dav.t_now;
		...
		*/
		if (   (   vcb->state == VCB_STATE_NONE
			...
			if (mf->motion_status & (MOTION_DIRECTION | MOTION_BURST))
				...
				dav.external_motion_record_event = TRUE;

			if (   !(vcb->state & VCB_STATE_LOOP_RECORD)
			    && !dav.motion_stills_enable
			   )
				...
				*/
				...
				if (*dav.on_motion_begin_cmd)
					event_add("motion begin", dav.t_now, 0,
							event_motion_begin_cmd,
							dav.on_motion_begin_cmd);
				}

			dav.video_notify = FALSE;
			dav.still_notify = FALSE;
			dav.timelapse_notify = FALSE;
			...
			if (dav.verbose_motion && !dav.verbose)
			...
			*/
			dav.motion_sync_time = dav.t_now
			...
			if (vcb->state & VCB_STATE_MOTION_RECORD)
				...
				{
				timersub(&dav.tv_now,
							&dav.motion_stills_start_tv, &tv_diff);
	...
	if (   dav.motion_stills_record
	    && dav.t_now >= dav.motion_stills_capture_time
...
motion_preview_area_fixup(void)
	...
	*/
	...
	if (frame_vec->x + dx / 2 >= dav.mjpeg_width)
		frame_vec->x = dav.mjpeg_width - dx / 2 - 1;
	...
	if (frame_vec->y + dy / 2 >= dav.mjpeg_height)
		frame_vec->y = dav.mjpeg_height - dy / 2 - 1;
...
regions_custom_config(char *custom)
	...
	if (*custom && strcmp(custom, "default"))
		asprintf(&fname, dav_MOTION_REGIONS_CUSTOM_CONFIG, custom);
	else
		asprintf(&fname, dav_MOTION_REGIONS_CONFIG);

	home_dir = getpwuid(geteuid())->pw_dir;
	asprintf(&path, "%s/%s/%s", home_dir, dav_CONFIG_DIR, fname);
	free(fname);
...
//		inform_message("Bad motion command.");
	...
	switch (id)
		...
		*/
		...
		case LIST_REGIONS:
			if ((dfd = opendir(dav.config_dir)) != NULL)
				{
				i = 2;
				n = 10;
		...
		*/
		...
		case SET_LIMITS:		/* limits magnitude count */
			...
			dav.motion_magnitude_limit_count = n;
		...
		case TRIGGER:
			...
			if (arg2[0])
				...
				if (arg3[0])
					...
					mf->fifo_trigger_time_limit = dav.motion_times.post_capture;
				}
			break;

		case CONFIRM_GAP:
			n = atoi(arg1);
			dav.motion_times.confirm_gap = n;
		...
		case PRE_CAPTURE:
			...
			if (vcb->state == VCB_STATE_NONE)
				{
				n = atoi(arg1);
				dav.motion_times.pre_capture = n;
		...
		case POST_CAPTURE:
			n = atoi(arg1);
			dav.motion_times.post_capture = n;
...
motion_init(void)
	...
	if (!done_once)
		{
		done_once = TRUE;
		motion_frame.motion_enable = dav.startup_motion_enable;
		}

	/* motion frames are from 16x16 macroblocks of the video frame
	*/
	motion_frame.width = ((dav.camera_config.video_width + 15) / 16) + 1;
	motion_frame.height = (dav.camera_config.video_height / 16) + 1;
...
motion_regions_config_load(char *config_file, boolean inform)
	...
	dup_string(&dav.motion_regions_name, reg_name);
	if (inform)
		{
