...
#define dav_CONFIG_DIR					".dav"
#define dav_CONFIG						"dav.conf"
#define dav_MOTION_REGIONS_CONFIG		"motion-regions.conf"
#define dav_MOTION_REGIONS_CUSTOM_CONFIG	"motion-regions-%s.conf"
#define dav_AT_COMMANDS_CONFIG			"at-commands.conf"
#define dav_TIMELAPSE_STATUS				"timelapse.status"

  /* These subdirs must match what is in www/config.php
  */
#define dav_VIDEO_SUBDIR					"videos"
#define dav_THUMBS_SUBDIR				"thumbs"
#define dav_STILL_SUBDIR					"stills"
#define dav_TIMELAPSE_SUBDIR				"timelapse"
...
#define	MOTION_VECTOR_TO_MJPEG_X(mvx) \
			((mvx) * 16 * dav.mjpeg_width / dav.camera_config.video_width)
#define	MOTION_VECTOR_TO_MJPEG_Y(mvy) \
			((mvy) * 16 * dav.mjpeg_height / dav.camera_config.video_height)

  /* Scale a mjpeg coordinate to a motion vector frame coordinate.
  */
#define	MJPEG_TO_MOTION_VECTOR_X(mjx) \
			((mjx) * dav.camera_config.video_width / 16 / dav.mjpeg_width)
#define	MJPEG_TO_MOTION_VECTOR_Y(mjy) \
			((mjy) * dav.camera_config.video_height / 16 / dav.mjpeg_height)
...
/* Motion types for writing the /run/dav/motion-events file
*/
