...
zoom_percent(int percent)
	...
	if (   percent < 10 || percent > 100
	    || (percent == dav.zoom_percent_prev && !force_crop)
	   )
		return;
	force_crop = FALSE;
	...
	dav.zoom_motion_holdoff = 5;
	crop_control_set(NULL, buf);
	dav.zoom_percent_prev = percent;
	}
...
static CameraParameter  camera_parameters[] =
	{
	{ "sharpness",  "0",   rational_control_set,  &dav.sharpness },
	{ "contrast",   "0",   rational_control_set,  &dav.contrast },
	{ "brightness", "50",  rational_control_set,  &dav.brightness },
	{ "saturation", "0",   rational_control_set,  &dav.saturation },

	{ "iso",           "0",  uint32_control_set,  &dav.iso },
	{ "shutter_speed", "0",  uint32_control_set,  &dav.shutter_speed },

	{ "exposure_compensation", "0", int32_control_set, &dav.exposure_compensation},

	{ "video_stabilisation", "false",  boolean_control_set,  &dav.video_stabilisation },
	{ "raw_capture",         "false",  boolean_control_set,  &dav.raw_capture },
//	{ "draw_focus_box",      "false",  boolean_control_set,  &dav.draw_focus_box },

	{ "rotation",      "0",    rotation_control_set, &dav.rotation },
	{ "exposure_mode", "auto", exposure_mode_set,    &dav.exposure_mode },
	{ "image_effect",  "none", image_effect_set,     &dav.image_effect },

	{ "hflip",  "off",    flip_control_set,  &dav.hflip },
	{ "vflip",  "off",    flip_control_set,  &dav.vflip },
	{ "crop",   "0 0 65536 65536", crop_control_set,  &dav.crop },

	{ "metering_mode", "average",       metering_mode_set, &dav.metering_mode },
	{ "white_balance", "auto",          awb_mode_set,      &dav.white_balance },
	{ "color_effect",  "false 128 128", color_effect_set,  &dav.color_effect }
	};
...
static Config  config[] =
	...
	{ "# ----------------------------------------------------------\n"
	  "#\n"
	  "# The dav installation directory.\n"
	  "# This must match where dav is installed and it is checked by\n"
	  "# the install-dav.sh script.  This should not need to be edited.\n"
	  "#",
	"install_dir", "/home/pi/dav", TRUE, { .string = &dav.install_dir }, config_string_set },
	...
	{ "# If media_dir has no leading '/' it will be a sub directory in install_dir.\n"
	  ...
	  "# So the default media dir is /home/pi/dav/media.\n"
	  ...
	  "#",
	"media_dir", "media", TRUE, { .string = &dav.media_dir }, config_string_set },
	...
	{ "# If archive_dir has no leading '/' it will be a sub directory under media_dir.\n"
	  ...
	  "# So the default archive dir is /home/pi/dav/media/archive.\n"
	  ...
	  "#",
	"archive_dir", "archive", TRUE, { .string = &dav.archive_dir }, config_string_set },
	...
	{ "# If loop_dir has no leading '/' it will be a sub directory under media_dir.\n"
	  ...
	  "# So the default archive dir is /home/pi/dav/media/loop.\n"
	  "# A file system may be mounted on the loop dir in the startup script.\n"
	  "#",
	"loop_dir", "loop", TRUE, { .string = &dav.loop_dir }, config_string_set },

	{ "# Log file.\n"
	  "#",
	"log_file",  "/tmp/dav.log", TRUE, { .string = &dav.log_file }, config_string_set },
	...
	{ "# At startup and at each new day, trim the log file number of lines\n"
	  ...
	  "#",
	"log_lines", "500", FALSE, {.value = &dav.log_lines}, config_value_int_set},

	{ "# Command to run at dav startup.  This is run after the config\n"
	  ...
	  "# is started, add a command to the file: ~/.dav/at-commands.conf.\n"
	  ...
	  "#",
	"on_startup", "$C/startup $I $m $a $G",  TRUE, {.string = &dav.on_startup_cmd}, config_string_set },

	{ "# Set to off to disable accepting halt and reboot commands\n"
	  "# from the FIFO and the web page.\n"
	  "#",
	"halt_enable",	"on", TRUE, {.value = &dav.halt_enable},  config_value_bool_set},

	{ "# Set to off to disable the multicast interface.\n"
	  "#",
	"multicast_enable",	"on", TRUE, {.value = &dav.multicast_enable},  config_value_bool_set},

	{ "\n# -------------------- Loop Record Options -----------------------\n"
	  "# Enable loop recording at startup.\n"
	  "#",
	"loop_enable",	"off", FALSE, {.value = &dav.loop_startup_enable},       config_value_bool_set},

	{ "# Time length limit of a loop video record.\n"
	  "#",
	"loop_record_time_limit",  "30", FALSE, {.value = &dav.loop_record_time_limit},      config_value_int_set},
	...
	{ "# Loop videos disk usage max percent (5 - 95).  Oldest loop videos are\n"
	  ...
	  "#",
	"loop_diskusage_percent",  "30", FALSE, {.value = &dav.loop_diskusage_percent},      config_value_int_set},

	{ "\n# -------------------- Motion Still Recording -----------------------\n"
	  "# Take still images instead of recording videos when motion is enabled.\n"
	  "#",
	"motion_stills_enable",	"off", FALSE, {.value = &dav.motion_stills_enable},       config_value_bool_set},
	...
	{ "# Motion still images per minute range from 1 to 60 which gives a\n"
	  ...
	  "#",
	"motion_stills_per_minute",  "30", TRUE, {.value = &dav.motion_stills_per_minute}, config_value_int_set },

	{ "\n# -------------------- Motion Detect Options -----------------------\n"
	  "# dav V3.0 stores some motion detect settings in preset-xxx.conf\n"
	  "# Vector and burst limits/counts are no longer saved in dav.conf.\n"
	  "#\n"
	  "# Enable dav motion detection at startup\n"
	  "#",
	"motion_enable",	"off", FALSE, {.value = &dav.startup_motion_enable},       config_value_bool_set},


	{ "# If off, do not detect motion when servos are off a preset.\n"
	  "#",
	"motion_off_preset",	"off", FALSE, {.value = &dav.motion_off_preset},       config_value_bool_set},

	{ "#OBSOLETE",
	"motion_magnitude_limit",  "5", FALSE, {.value = &dav.motion_magnitude_limit},      config_value_int_set},
	{ "#OBSOLETE",
	"motion_magnitude_limit_count",  "4", FALSE, {.value = &dav.motion_magnitude_limit_count},      config_value_int_set},
	{ "#OBSOLETE",
	"motion_burst_count",  "400", FALSE, {.value = &dav.motion_burst_count},      config_value_int_set},
	{ "#OBSOLETE",
	"motion_burst_frames",  "3", FALSE, {.value = &dav.motion_burst_frames},      config_value_int_set},
	...
	{ "# Time length limit of motion video record excluding pre_capture time.\n"
	  ...
	  "#",
	"motion_record_time_limit",  "0", FALSE, {.value = &dav.motion_record_time_limit},      config_value_int_set},

	{ "# Percent to dim image when drawing motion vectors.  Range 30 - 60\n"
	  "#",
	"motion_vectors_dimming", "45", FALSE, {.value = &dav.motion_vectors_dimming}, config_value_int_set},
	...
	{ "# Require a second motion detect within this period of seconds before\n"
	  ...
	  "#",
	"motion_confirm_gap",   "4", TRUE, {.value = &dav.motion_times.confirm_gap},  config_value_int_set },
	...
	{ "# Command/script to run when a motion detect record ends.\n"
	  ...
	  "#",
	"on_motion_end",    "", TRUE, {.string = &dav.on_motion_end_cmd}, config_string_set },
	...
	{ "# Command/script to run when a motion enable changes.\n"
	  ...
	  "#",
	"on_motion_enable",  "", TRUE, {.string = &dav.on_motion_enable_cmd}, config_string_set },
	...
	{ "# Command/script to run when a loop record ends.\n"
	  ...
	  "#",
	"on_loop_end",    "", TRUE, {.string = &dav.on_loop_end_cmd}, config_string_set },

	{ "# Command/script to run when a manual record ends.\n"
	  "#",
	"on_manual_end",    "", TRUE, {.string = &dav.on_manual_end_cmd}, config_string_set },
	...
	{ "# Command to run on the motion preview jpeg file.\n"
	  ...
	  "#     on_motion_preview_save mpack -s dav@$H $F myuser@gmail.com\n"
	  ...
	  "#",
	"on_motion_preview_save", "", TRUE, {.string = &dav.on_motion_preview_save_cmd}, config_string_set },
	...
	{ "# Set to off if you want jpeg preview files to show the OSD overlay text\n"
	  ...
	  "#",
	"motion_preview_clean",  "on", FALSE, {.value = &dav.motion_preview_clean}, config_value_bool_set },

	{ "# If on, show extra vector count data on the OSD when presets are shown.\n"
	  "#",
	"motion_show_counts",  "off", FALSE, {.value = &dav.motion_show_counts}, config_value_bool_set },

	{ "# If on, show a warning if the preview mjpeg encoder stalls.\n"
	  "#",
	"preview_stall_warning",  "on", FALSE, {.value = &dav.preview_stall_warning}, config_value_bool_set },
	...
	{ "# Minimum width and height in pixels for the substitution width and height\n"
	  ...
	  "#",
	"motion_area_min_side",  "80", FALSE, {.value = &dav.motion_area_min_side}, config_value_int_set },
	...
	{ "# Enable writing a motion statistics .csv file for each motion video.\n"
	  ...
	  "#",
	"motion_stats",  "off", FALSE, {.value = &dav.motion_stats}, config_value_bool_set },

	{ "# Enable writing all motion detects to ~/dav/www/motion_detects_fifo\n"
	  ...
	  "#",
	"motion_detects_fifo_enable",  "off", FALSE, {.value = &dav.motion_detects_fifo_enable}, config_value_bool_set },

	{ "# Command/script to run when receiving a user defined multicast\n"
	  "# pkc-message sent by other davs or separate scripts on your LAN.\n"
	  "# Use this to run a script needing dav variables passed to it,\n"
	  ...
	  "# dav itself accepts message types of \"command\" and \"pkc-message\".\n"
	  "# See the help page.\n"
	  "#",
	"on_multicast_pkc_message",  "", TRUE, {.string = &dav.on_multicast_message_cmd}, config_string_set },


	{ "\n# -------------------- Audio Trigger Options -----------------------\n"
	  "# Enable audio events to trigger a motion video.\n"
	  "#",
	"audio_trigger_video",	"off", FALSE, {.value = &dav.audio_trigger_video},       config_value_bool_set},

	{ "# Audio level to trigger a motion video, range 2 - 100\n"
	  "#",
	"audio_trigger_level",  "50", FALSE, {.value = &dav.audio_trigger_level},      config_value_int_set},
	...
	{ "# If this is on and an there is no video motion detected during a video\n"
	  ...
	  "# See the help page.\n"
	  "#",
	"box_MP3_only",  "off", FALSE, {.value = &dav.audio_box_MP3_only}, config_value_bool_set },
	...
	{ "\n# --------------------- Video Record Options -----------------------\n"
	  ...
	  "# have a way to manage the videos.  dav webpages do not support\n"
	  "# or manage .h264 videos.\n"
	  "#",
	"video_motion_name_format", "motion_%F_%H.%M.%S_$N.mp4", TRUE,
		{.string = &dav.video_motion_name_format}, config_string_set },
	...
	"video_manual_name_format", "manual_%F_%H.%M.%S_$N.mp4", TRUE,
		{.string = &dav.video_manual_name_format}, config_string_set },
	...
	{ "# Disk free percent (5 - 90).  Loop vidoes and, if enabled, media videos\n"
	  ...
	  "#",
	"diskfree_percent",  "20", FALSE, {.value = &dav.diskfree_percent},      config_value_int_set},

	{ "# If on, check the media file system as each video is recorded and\n"
	  "# delete the oldest media to maintain the free percent.\n"
	  "#",
	"check_media_diskfree",  "off", FALSE, {.value = &dav.check_media_diskfree}, config_value_bool_set },
	...
	{ "# If on, check the archive file system as each video is archived and\n"
	  ...
	  "#",
	"check_archive_diskfree",  "off", FALSE, {.value = &dav.check_archive_diskfree}, config_value_bool_set },

	{ "# Pixel width of videos recorded.\n"
	  "#",
	"video_width",    "1920", TRUE, {.value = &dav.camera_config.video_width},      config_value_int_set },

	{ "# Pixel height of videos recorded.\n"
	  "#",
	"video_height",   "1080", TRUE, {.value = &dav.camera_config.video_height},     config_value_int_set },
	...
	{ "# Video frames per second.  If set higher than 24 the motion detecting\n"
	  ...
	  "#",
	"video_fps",      "24", FALSE, {.value = &dav.camera_adjust.video_fps},        config_value_int_set },
	...
	{ "# MP4Box output frames per second if video filename is a .mp4\n"
	  ...
	  "#",
	"video_mp4box_fps", "0", FALSE, {.value_float = &dav.camera_adjust.video_mp4box_fps},        config_value_float_set },
	...
	{ "# Video bitrate affects the quality and size of a video recording.\n"
	  ...
	  "# dav video circular buffer memory usage.\n"
	  ...
	  "#",
	"video_bitrate",  "6000000", TRUE, {.value = &dav.camera_adjust.video_bitrate},    config_value_int_set },

	{ "# Pixel width of the streamed jpeg file /run/dav/mjpeg.jpg.\n"
	  ...
	  "#",
	"mjpeg_width",    "800",  TRUE, {.value = &dav.mjpeg_width},      config_value_int_set },
	...
	{ "# Quality factor (up to 100) affects the quality and size of the stream jpeg.\n"
	  ...
	  "#",
	"mjpeg_quality",  "8",  TRUE, {.value = &dav.mjpeg_quality},    config_value_int_set },
	...
	{ "# Divide the video_fps by this to get the stream jpeg file update rate.\n"
	  ...
	  "#",
	"mjpeg_divider",  "4", FALSE, {.value = &dav.mjpeg_divider},    config_value_int_set },
	...
	{ "\n# ------------------ Still Capture Options -----------------------\n"
	  ...
	  "#",
	"still_name_format", "image_%F_%H.%M.%S_$N.jpg", TRUE, {.string = &dav.still_name_format}, config_string_set },

	{ "# Width of a still capture in pixels.  Max value 2592\n"
	  "#",
	"still_width",   "1920", TRUE, {.value = &dav.camera_config.still_width}, config_value_int_set },

	{ "# Height of a still capture in pixels.  Max value 1944\n"
	  "#",
	"still_height",  "1080", TRUE, {.value = &dav.camera_config.still_height}, config_value_int_set },

	{ "# This quality factor affects the size and quality of still captures.\n"
	  "#",
	"still_quality", "14", TRUE, {.value = &dav.camera_adjust.still_quality},   config_value_int_set },

	{ "# Command to run after a still capture.\n"
	  "# email the still somewhere with:\n"
	  "#   on_still_capture mpack -s dav@$H $s myuser@gmail.com\n"
	  ...
	  "#",
	"on_still_capture", "", TRUE, {.string = &dav.on_still_capture_cmd}, config_string_set },
	...
	{ "# Timelapse name format.\n"
	  ...
	  "#",
	"video_timelapse_name_format", "tl_%F_%H.%M.%S_$n.mp4", TRUE, {.string = &dav.video_timelapse_name_format}, config_string_set },
	...
	"timelapse_convert", "$c/_timelapse-convert $m $T $n $G $P $l", TRUE,
						{.string = &dav.timelapse_convert_cmd}, config_string_set },


	{ "\n# ------------------- Servo/Preset Options  -----------------------\n"
	  "#\n"
	  "# dav can use internal hardware PWM code to drive servos and for\n"
	  ...
	  "# \n"
	  "# Or, dav can use ServoBlaster and will then send servo commands\n"
	  "# to /dev/servoblaster.  But for this, a separate ServoBlaster install\n"
	  "# and configuration to run is required.\n"
	  "# For ServoBlaster, the dav servo pan/tilt gpio values should not\n"
	  ...
	  "#",
	"servo_pan_gpio", "-1", FALSE, {.value = &dav.servo_pan_gpio}, config_value_int_set },

	{ "#",
	"servo_tilt_gpio", "-1", FALSE, {.value = &dav.servo_tilt_gpio}, config_value_int_set },

	{ "# Set to true to use ServoBlaster for servos.  A separate install of\n"
	  "# ServoBlaster will be required.\n"
	  "#",
	"servo_use_servoblaster",  "false", TRUE, {.value = &dav.servo_use_servoblaster }, config_value_bool_set },
	...
	{ "# pan/tilt min/max values are best set using the web OSD.\n"
	  ...
	  "#",
	"servo_pan_min", "120", FALSE, {.value = &dav.servo_pan_min}, config_value_int_set },

	{ "#",
	"servo_pan_max", "180", FALSE, {.value = &dav.servo_pan_max}, config_value_int_set },

	{ "#",
	"servo_tilt_min", "130", FALSE, {.value = &dav.servo_tilt_min}, config_value_int_set },

	{ "#",
	"servo_tilt_max", "170", FALSE, {.value = &dav.servo_tilt_max}, config_value_int_set },

	{ "# Set invert values to on if the servo turns the wrong way.\n"
	  "#",
	"servo_pan_invert",  "off", FALSE, {.value = &dav.servo_pan_invert}, config_value_bool_set },

	{ "#",
	"servo_tilt_invert",  "off", FALSE, {.value = &dav.servo_tilt_invert}, config_value_bool_set },
	...
	{ "# Servo moves have three modes: move by one step, by servo_move_steps,\n"
	  ...
	  "#",
	"servo_move_steps",  "10", FALSE, {.value = &dav.servo_move_steps }, config_value_int_set },

	{ "# Delay in msec between servo pulse width steps for servo move commands.\n"
	  "#",
	"servo_move_step_msec",  "40", FALSE, {.value = &dav.servo_move_step_msec }, config_value_int_set },

	{ "# Delay in msec between servo pulse width steps when going to a preset.\n"
	  "#",
	"servo_preset_step_msec",  "20", FALSE, {.value = &dav.servo_preset_step_msec }, config_value_int_set },

	{ "# Delay in msec after servo stops moving before enabling motion detection.\n"
	  "#",
	"servo_settle_msec",  "600", FALSE, {.value = &dav.servo_settle_msec }, config_value_int_set },
	...
	{ "\n# ------------------- Audio Options  -----------------------\n"
	  ...
	  "#",
	"audio_enable",  "false", TRUE, {.value = &dav.audio_enable }, config_value_bool_set },
	...
	{ "# ALSA hardware audio input (microphone) capture device. Using the hw:N\n"
	  ...
	  "#",
	"audio_device",  "plughw:1", FALSE, {.string = &dav.audio_device },   config_string_set },
	...
	{ "# Audio rate.  See Help page for info on this and remaining audio options.\n"
	  ...
	  "#",
	"audio_rate_Pi2", "48000", FALSE, {.value = &dav.audio_rate_Pi2}, config_value_int_set },

	{ "# Audio rate for a Pi model 1 (armv6 single core Pi1).\n"
	  "#",
	"audio_rate_Pi1", "24000", FALSE, {.value = &dav.audio_rate_Pi1}, config_value_int_set },
	...
	{ "# Audio channels.  A USB sound card probably supports only mono and\n"
	  ...
	  "#",
	"audio_channels", "1", FALSE, {.value = &dav.audio_channels}, config_value_int_set },

	{ "# Microphone audio gain dB (0 - 30). Set using the web page audio\n"
	  "# gain up/down control buttons.\n"
	  "#",
	"audio_gain_dB", "0", FALSE, {.value = &dav.audio_gain_dB}, config_value_int_set },

	{ "# MP3 lame encode quality for a Pi2/3, range 0 - 9.\n"
	  "#",
	"audio_mp3_quality_Pi2", "2", FALSE, {.value = &dav.audio_mp3_quality_Pi2}, config_value_int_set },

	{ "# MP3 lame encode quality for a Pi1, range 0 - 9.\n"
	  "#",
	"audio_mp3_quality_Pi1", "7", FALSE, {.value = &dav.audio_mp3_quality_Pi1}, config_value_int_set },
	...
	{ "\n# ------------------- Miscellaneous Options  -----------------------\n"
	  ...
	  "#",
	"notify_duration", "4", FALSE, {.value = &dav.notify_duration}, config_value_int_set },

	{ "# Your latitude used to calculate sun rise, set, dawn, and dusk times.\n"
	  "#",
	"latitude",  "30.12N", FALSE, {.string = &dav.latitude },   config_string_set },

	{ "# Your longitude used to calculate sun rise, set, dawn, and dusk times.\n"
	  "#",
	"longitude", "97.88W", FALSE, {.string = &dav.longitude },   config_string_set },

	{ "# Setting a locale to get annotated time shown in your language is\n"
	  "# not preserved when starting dav from the web page.  Set it\n"
	  "# here for dav to explicitely set it.\n"
	  "# \n"
	  "# Example:  lc_time es_ES.utf8",
	"lc_time", "", FALSE, {.string = &dav.lc_time },   config_string_set },

	{ "# Do not edit.  Used internally to force startup config write if needed.\n"
	  "#",
	"config_sequence", "0", FALSE, {.value = &dav.config_sequence}, config_value_int_set },
	...
	"annotate_format_string",  "%a %b %e, %l:%M.%S %p", FALSE,
	                           {.string = &dav.annotate_format_string}, config_string_set },

	{ "# Enables drawing the annotate date string.\n"
	  "#",
	"annotate_enable",      "on", FALSE, {.value = &dav.annotate_enable},      config_value_bool_set },

	{ "# Extra information drawn on the video.\n"
	  "#",
	"annotate_show_motion", "off", FALSE, {.value = &dav.annotate_show_motion}, config_value_bool_set },

	{ "# Extra information drawn on the video.\n"
	  "#",
	"annotate_show_frame",  "off", FALSE, {.value = &dav.annotate_show_frame},  config_value_bool_set },
	...
	{ "# Annotate text background color.  Set to \"none\" for no background.\n"
	  ...
	  "#",
	"annotate_text_background_color",    "808080", FALSE, {.string = &dav.annotate_text_background_color },   config_string_set },

	{ "# Annotate text brightness. Range: integer from 0 - 255\n"
	  "# Text cannot be set to a color, only to a brightness..\n"
	  "#",
	"annotate_text_brightness",    "255", FALSE, {.value = &dav.annotate_text_brightness },   config_value_int_set },

	{ "# Annotate text size. Range: integer from 6 - 160\n"
	  "#",
	"annotate_text_size",    "40", FALSE, {.value = &dav.annotate_text_size },   config_value_int_set },
	};
...
config_set_defaults(char *home_dir)
	...
	/* Camera parameter table and dav config table have initial value pointers
	...
	*/
	...
	dav.version = strdup(dav_VERSION);
	dav.timelapse_format = strdup("tl_$n_$N.jpg");
	dav.motion_stills_name_format = strdup("%F_%H.%M.%S_$N.jpg");

	dav.preview_pathname = strdup("");
	dav.thumb_name = strdup("");
	dav.multicast_group_IP = "225.0.0.55";
	dav.multicast_group_port = 22555;
	gethostname(dav.hostname, HOST_NAME_MAX);	

	/* If dav started by rc.local or web page, need to get correct
	|  home directory.  Makefile does a setuid/setgid on executable.
	*/
	if (!home_dir)
		home_dir = getpwuid(geteuid())->pw_dir;
	asprintf(&dav.config_dir, "%s/%s", home_dir, dav_CONFIG_DIR);
	dav.tmpfs_dir = strdup("/run/dav");
	asprintf(&dav.motion_events_filename,
				"%s/motion-events", dav.tmpfs_dir);

	if (make_directory(dav.config_dir))
		{
		asprintf(&dav.config_file, "%s/%s",
					dav.config_dir, dav_CONFIG);
		asprintf(&dav.motion_regions_config_file, "%s/%s",
					dav.config_dir, dav_MOTION_REGIONS_CONFIG);
		asprintf(&dav.at_commands_config_file, "%s/%s",
					dav.config_dir, dav_AT_COMMANDS_CONFIG);
		asprintf(&dav.timelapse_status_file, "%s/%s",
					dav.config_dir, dav_TIMELAPSE_STATUS);
		asprintf(&dav.media_sequence_file, "%s/%s",
					dav.config_dir, "media-sequence");
		}
...
config_load(char *config_file)
	...
	dav.mjpeg_width = (dav.mjpeg_width + 8) & ~0xf;

	if (dav.motion_magnitude_limit < 2)
		dav.motion_magnitude_limit = 2;
	if (dav.motion_magnitude_limit_count < 2)
		dav.motion_magnitude_limit_count = 2;

	if (dav.motion_burst_count < 20)
		dav.motion_burst_count = 20;
	if (dav.motion_burst_frames < 2)
		dav.motion_burst_frames = 2;

	if (   dav.motion_record_time_limit != 0
	    && dav.motion_record_time_limit < 10
	   )
		dav.motion_record_time_limit = 10;

	dav.zoom_percent = 100;
	dav.zoom_percent_prev = 100;

	if (dav.motion_stills_per_minute > 60)
		dav.motion_stills_per_minute = 60;
	else if (dav.motion_stills_per_minute < 1)
		dav.motion_stills_per_minute = 1;

	if (dav.diskfree_percent < 5)
		dav.diskfree_percent = 5;
	if (dav.loop_diskusage_percent < 5)
		dav.loop_diskusage_percent = 5;


	if (dav.diskfree_percent > 90)
		dav.diskfree_percent = 90;
	if (dav.loop_diskusage_percent > 95)
		dav.loop_diskusage_percent = 95;

	if (dav.loop_record_time_limit < 10)
		dav.loop_record_time_limit = 10;

	if (dav.motion_vectors_dimming < 30)
		dav.motion_vectors_dimming = 30;
	if (dav.motion_vectors_dimming > 60)
		dav.motion_vectors_dimming = 60;

	if (dav.motion_times.post_capture > dav.motion_times.event_gap)
		dav.motion_times.event_gap = dav.motion_times.post_capture;

	if (dav.audio_trigger_level < 2)
		dav.audio_trigger_level = 2;
	if (dav.audio_trigger_level > 100)
		dav.audio_trigger_level = 100;

	if (dav.annotate_text_size < 6)
		dav.annotate_text_size = 6;
	else if (dav.annotate_text_size > 160)
		dav.annotate_text_size = 160;
	if (dav.annotate_text_brightness < 0)
		dav.annotate_text_size = 0;
	else if (dav.annotate_text_brightness > 255)
		dav.annotate_text_size = 255;

	if (dav.audio_gain_dB > 30)
		dav.audio_gain_dB = 30;
	else if (dav.audio_gain_dB < 0)
		dav.audio_gain_dB = 0;


	dav.annotate_string_space_char = '_';

	n = (int) ((dav.camera_adjust.video_mp4box_fps + .0005) * 100.0);
	if (n < 0)
		n = 0;
	if (n > 3000)
		n = 3000;
	dav.camera_adjust.video_mp4box_fps_display = n;

	camera_adjust_temp = dav.camera_adjust;
	motion_times_temp = dav.motion_times;

	if (   (dav.servo_use_servoblaster && dav.servo_pan_gpio >= 0)
	    || (   !dav.servo_use_servoblaster
	        && (   dav.servo_pan_gpio == 12 || dav.servo_pan_gpio == 13
	            || dav.servo_pan_gpio == 18 || dav.servo_pan_gpio == 19
	           )
	       )
	   )
		dav.have_servos = TRUE;

	dav.loop_name_format = strdup("loop_%F_%H.%M.%S_$N.mp4");

	asprintf(&dav.preset_config_file, "%s/preset-%s.conf",
					dav.config_dir,
					dav.have_servos ? "servos" : "no-servos");
	asprintf(&dav.preset_state_file, "%s/preset-%s.state",
					dav.config_dir,
					dav.have_servos ? "servos" : "no-servos");

	if (dav.config_sequence_new != dav.config_sequence)
		{
		dav.config_sequence = dav.config_sequence_new;
		dav.config_modified = TRUE;
		}
	return TRUE;
	}
...
config_save(char *config_file)
	...
	if (!f)
		{
		printf("Failed to save dav config file: %s.  %m\n", config_file);
		return;
		}
	...
	fprintf(f,
"# ==================================================================\n"
"#               dav Program Configuration\n"
"#\n"
"# When editing this file, stop and then restart dav so your changes\n"
"# will be in effect.  Otherwise, dav may overwrite this file and your\n"
"# changes will be lost.\n"
"# Commands in this file should not be enclosed in quotes and commands may\n"
"# have substitution variables.\n"
"# See ~/.dav/at_commands.conf for a list of the substitution variables.\n"
...
config_timelapse_save_status(void)
	...
	f = fopen(dav.timelapse_status_file, "w");
	if (f)
		{
...
config_timelapse_load_status(void)
	...
	f = fopen(dav.timelapse_status_file, "r");
	if (f)
		{
...
config_media_sequence_save(void)
	...
	f = fopen(dav.media_sequence_file, "w");
	if (f)
		{
		strftime(buf, sizeof(buf), "%F", localtime(&dav.t_now));
		fprintf(f, "%s\n", buf);
		fprintf(f, "motion_videos %d\n", dav.video_motion_sequence);
		fprintf(f, "manual_videos %d\n", dav.video_manual_sequence);
		fprintf(f, "motion_stills %d\n", dav.motion_stills_sequence);
		fprintf(f, "manual_stills %d\n", dav.still_sequence);
		fclose(f);
		}
	dav.config_media_sequence_modified = FALSE;
	}
...
config_media_sequence_load(void)
	...
	dav.video_motion_sequence = 0;
	dav.video_manual_sequence = 0;
	dav.motion_stills_sequence = 0;
	dav.still_sequence = 0;
	f = fopen(dav.media_sequence_file, "r");
	if (f)
		{
		fgets(buf, sizeof(buf), f);
		...
		if (!strcmp(buf, buf1))
			...
			while (fgets(buf, sizeof(buf), f) != NULL)
				...
				if (!strcmp(tag, "motion_videos"))
					dav.video_motion_sequence = seq;
				else if (!strcmp(tag, "manual_videos"))
					dav.video_manual_sequence = seq;
				else if (!strcmp(tag, "motion_stills"))
					dav.motion_stills_sequence = seq;
				else if (!strcmp(tag, "manual_stills"))
					dav.still_sequence = seq;
				}
			}
		fclose(f);
		}
