...
static Sun	sun;
  ...
  |  so only dav specific $X conversions remain.  Change all '$X' to
		...
		*fmt = 's';
		switch (specifier)
			...
			case 'F':
				...
				break;
			case 'H':
				fmt_arg = dav.hostname;
				break;
			case 'h':
				fmt_arg = dav.multicast_from_hostname;
				break;
			case 'E':
				fmt_arg = dav.effective_user;
				break;
			case 'I':
				fmt_arg = dav.install_dir;
				break;
			case 's':
				fmt_arg = dav.still_last;
				break;
			case 'S':
				fmt_arg = dav.still_dir;
				break;
			case 'X':
				snprintf(buf, sizeof(buf), "%d", dav.video_motion_sequence);
				fmt_arg = buf;
				break;
			case 'v':
				fmt_arg = dav.video_last;
				break;
			case 'V':
				fmt_arg = dav.video_dir;
				break;
			case 't':
				fmt_arg = dav.thumb_dir;
				break;
			case 'T':
				snprintf(buf, sizeof(buf), "%d", time_lapse.period);
				name = media_pathname(dav.video_dir,
						dav.video_timelapse_name_format, 0,
						'n',  buf, 'H', dav.hostname);
				snprintf(buf, sizeof(buf), "%s", name);
				free(name);
				fmt_arg = buf;
				dup_string(&dav.timelapse_video_pending, fmt_arg);
				break;
			case 'l':
				snprintf(buf, sizeof(buf), "%05d", time_lapse.series);
				name = strdup(dav.timelapse_format);
				...
				break;
			case 'L':
				fmt_arg = dav.timelapse_dir;
				break;
			case 'a':
				fmt_arg = dav.archive_dir;
				break;
			case 'z':
				fmt_arg = dav.loop_dir;
				break;
			case 'm':
				fmt_arg = dav.media_dir;
				break;
			case 'M':
				fmt_arg = dav.mjpeg_filename;
				break;
			case 'P':
				fmt_arg = dav.command_fifo;
				break;
			case 'p':
				pos = (PresetPosition *) slist_nth_data(dav.preset_position_list,
						dav.preset_position_index);
				snprintf(buf, sizeof(buf), "%d %d",
						dav.preset_position_index + 1,
						pos ? pos->settings_index + 1 : 1);
				fmt_arg = buf;
				break;
			case 'c':
				fmt_arg = dav.scripts_dist_dir;
				break;
			case 'C':
				fmt_arg = dav.scripts_dir;
				break;
			case 'G':
				fmt_arg = dav.log_file;
				break;
			case 'N':
			...
			case 'A':
				if (vcb->state & VCB_STATE_LOOP_RECORD)
					dir = dav.loop_thumb_dir;
				else if (vcb->state & VCB_STATE_MANUAL_RECORD)
					dir = dav.thumb_dir;
				else if (dav.motion_stills_enable)
					dir = dav.still_thumb_dir;
				else	/* VCB_STATE_MOTION_RECORD */
					dir = dav.thumb_dir;
				snprintf(buf, sizeof(buf), "%s/%s", dir, dav.thumb_name);
				fmt_arg = buf;
				break;
			case 'r':
				if (dav.motion_stills_enable)
					fmt_arg = "stills";
				else
					fmt_arg = "video";
				break;
			case 'e':
				if (dav.external_motion_record_event)
					snprintf(buf, sizeof(buf), "%s",
...
event_motion_enable_cmd(char *cmd)
	...
	exec_no_wait(cmd, NULL, dav.verbose_log);
	}
...
event_motion_still_capture(void *p)
	{
	if (!dav.motion_still_pathname)
		return;
	still_capture(dav.motion_still_pathname, TRUE);
	free(dav.motion_still_pathname);
	dav.motion_still_pathname = NULL;
	}
...
diskfree_is_low(char *dir)
	...
	is_low = (free_pct <= dav.diskfree_percent);
	if (is_low && dav.verbose_log)
		log_printf("%s: free space %d <= Diskfree_percent limit %d\n",
					dir, free_pct, dav.diskfree_percent);
	return is_low;
	}
...
event_video_diskfree_percent(char *type)
	{
	video_diskfree_percent_delete(dav.media_dir, NULL);
	}
...
event_still_diskfree_percent(char *subdir)
	{
	still_diskfree_percent_delete(dav.media_dir, subdir, NULL);
	}
...
event_archive_diskfree_percent(char *type)
	...
	if (!diskfree_is_low(dav.archive_dir))
		return;
	filter_parent = dav.archive_dir;
	n_yr = scandir(dav.archive_dir, &year_list, dir_filter, alphasort);
	for (y = 0; y < n_yr; ++y)
		{
		snprintf(year_dir, sizeof(year_dir), "%s/%s",
					dav.archive_dir, year_list[y]->d_name);
...
event_loop_diskusage_percent(void)
	...
	*/ 
	if ((dir = opendir(dav.loop_video_dir)) != NULL)
		{
		for (entry = readdir(dir); entry; entry = readdir(dir))
			...
			if (!strcmp(entry->d_name, ".") || !strcmp(entry->d_name, ".."))
				continue;
			snprintf(fname, sizeof(fname), "%s/%s",
						dav.loop_video_dir, entry->d_name);
			if (lstat(fname, &sb) == 0)
				used_blocks += sb.st_blocks;
			}
		closedir(dir);
		}
	if ((dir = opendir(dav.loop_thumb_dir)) != NULL)
		{
		for (entry = readdir(dir); entry; entry = readdir(dir))
			{
			if (!strcmp(entry->d_name, ".") || !strcmp(entry->d_name, ".."))
				continue;
			snprintf(fname, sizeof(fname), "%s/%s",
						dav.loop_thumb_dir, entry->d_name);
			if (lstat(fname, &sb) == 0)
				used_blocks += sb.st_blocks;
			}
		closedir(dir);
	...
	diskfree_low = diskfree_is_low(dav.loop_dir);

	if (used_percent < dav.loop_diskusage_percent && !diskfree_low)
		return;

	n = scandir(dav.loop_video_dir, &mp4_list,
	...
	for (i = 0; i < n; ++i)
		{
		if (   used_percent >= dav.loop_diskusage_percent
		    || diskfree_low
		   )
			{
			if (dav.verbose_log)
			...
			snprintf(fname, sizeof(fname), "%s/%s",
						dav.loop_video_dir, mp4_list[i]->d_name);
			if (lstat(fname, &sb) == 0)
				used_blocks -= sb.st_blocks;
			unlink(fname);

			snprintf(fname, sizeof(fname) - 3, "%s/%s",
						dav.loop_thumb_dir, mp4_list[i]->d_name);
			if ((s = strstr(fname, ".mp4")) != NULL)
				{
				strcpy(s, ".th.jpg");
				if (lstat(fname, &sb) == 0)
...
state_file_write(void)
	...
	fprintf(f, "video_last %s\n",
			dav.video_last ? dav.video_last : "none");
	fprintf(f, "video_last_frame_count %d\n", dav.video_last_frame_count);
	...
	fprintf(f, "still_last %s\n",
			dav.still_last ? dav.still_last : "none");
	...
	fprintf(f, "timelapse_jpeg_last %s\n",
			(dav.timelapse_jpeg_last && *dav.timelapse_jpeg_last)
					? dav.timelapse_jpeg_last : "none");
	...
	fprintf(f, "timelapse_video_last %s\n",
			dav.timelapse_video_last ? dav.timelapse_video_last : "none");

	fprintf(f, "motion_detects_fifo_enable %s\n",
			dav.motion_detects_fifo_enable ? "on" : "off");

	fprintf(f, "current_minute %d\n",
			dav.tm_local.tm_hour * 60 + dav.tm_local.tm_min);
	fprintf(f, "dawn %d\n", sun.dawn);
	fprintf(f, "sunrise %d\n", sun.sunrise);
	fprintf(f, "sunset %d\n", sun.sunset);
	fprintf(f, "dusk %d\n", sun.dusk);

	fprintf(f, "multicast_group_IP %s\n", dav.multicast_group_IP);
	fprintf(f, "multicast_group_port %d\n", dav.multicast_group_port);

	fclose(f);
	rename(fname_part, dav.state_filename);
	}
...
sun_times_init(void)
	{
	struct tm	*tm = &dav.tm_local;
	int		n, year, month, mday;

	if (   sscanf(dav.latitude, "%lf", &sun.latitude) == 1
	    && sscanf(dav.longitude, "%lf", &sun.longitude) == 1
	   )
		{
		n = strlen(dav.latitude);
		if (dav.latitude[n - 1] == 'S' || dav.latitude[n - 1] == 's')
			sun.latitude = -sun.latitude;
		n = strlen(dav.longitude);
		if (dav.longitude[n - 1] == 'W' || dav.longitude[n - 1] == 'w')
...
event_process(void)
	...
	dav.second_tick = (dav.t_now == t_prev) ? FALSE : TRUE;
	t_prev = dav.t_now;

	if (dav.config_media_sequence_modified)
		config_media_sequence_save();

	if (dav.second_tick)
		{
		tm_prev = dav.tm_local;
		tm_now = localtime(&dav.t_now);
	...
	if (dav.state_modified || minute_tick)
		{
		dav.state_modified = FALSE;
		state_file_write();
		}
	...
	for (list = event_list; list; prev_link = list, list = list->next)
		...
		*/
		...
		if (event->time > 0 && event->time > dav.t_now)
			continue;
	...
	if (minute_tick)
		...
		if (at_notify_fd == 0)
			...
			if (at_notify_fd > 0)
				...
				at_notify_wd = inotify_add_watch(at_notify_fd,
						dav.config_dir, IN_CREATE | IN_MODIFY);
				}
			}
			...
			if (n > 0)
				...
				for (i = 0; i < n; i += sizeof(*event) + event->len)
					...
					if (   event->len > 0
					    && !strcmp(event->name,  dav_AT_COMMANDS_CONFIG)
					   )
						at_commands_config_load(dav.at_commands_config_file);
					}
				}
			}
		...
		if (dav.preset_state_modified && five_minute_tick)
			{
			dav.preset_state_modified = FALSE;
			preset_state_save();
			}
		...
		if (day_tick || !sun.initialized)
			...
			if (sun.initialized)
				...
				strftime(tbuf, sizeof(tbuf), "%F", localtime(&dav.t_now));
			...
			*/
			if (   !strcmp(at->frequency, "daily")
				...
				if (*(at->command) == '@')
					...
					cmd = substitute_var(cmd, 'H', dav.hostname);
					command_process(cmd);
					free(cmd);
					}
				else
...
at_commands_config_save(char *config_file)
	...
	"# Commands may be system commands/scripts or may be internal dav\n"
	...
	"#\n"
	"# If this file is modified, dav will automatically reload it.\n"
	"#\n"
	);
