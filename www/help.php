...
$header .= "<link rel=\"stylesheet\" href=\"js-css/dav.css\" />";
$header .= "</head>";
...
<div class='indent0'>
For an overview description of dav, visit the
<a href="http://billw2.github.io/dav/dav.html">dav website</a><br>
And there is a Raspberry Pi
<a href="https://www.raspberrypi.org/forums/viewtopic.php?f=43&t=115583">dav forum</a>
</div>
...
Version 4.1.4
<div class='indent1'>
Bug fix for dav Start from web page: use installing user instead
of hard wired "pi".<br>
New scripts-dist/example-motion-events demo of processing
<nobr>/run/dav/motion-events.</nobr><br>
...
dav.conf: on_manual_end on_loop_end on_motion_enable.
</div>
...
<div class='indent0'>
dav is installed from a github git repository using the command line. The install
...
<pre>
cd /home/pi
git clone https://github.com/billw2/dav.git
cd dav
./install-dav.sh
</pre>
The install-dav.sh script installs needed packages and prompts for
three things to configure:
	<ul>
	<li>Port number for the nginx web server
	to listen on.  The default is 80, but an alternate non standard port number can
	be used in which case the dav web page would be accessed with the URL:<br>
	&nbsp; &nbsp; &nbsp;<span style='font-weight:700'>http://your_pi:port_number</span>
	</li>
	<p>
	<li>Auto start: if enabled a line will be added to
	<span style='font-weight:700'>/etc/rc.local</span> so that dav will be
	auto started at boot.  If this is not enabled, dav will need to be started from
...
<div class='indent1'>
Go to the dav web page in your browser (omit the port number if it was left at default 80):<br>
&nbsp; &nbsp; &nbsp;<span style='font-weight:700'>http://your_pi:port_number</span>
	<ul>
	<li>Expand the
		<span style='font-weight:700'>System</span> panel and start the dav prgram
		...
		If this happens, you should fix the issue with the camera and restart dav.
	</li>
	<p>
	...
	<li>The OSD then shows that motion detection is
		...
		and dav is now operating with its default settings.
	</li>
	<p>
	...
	<li>On the button bar, click the buttons
		...
		<br>to toggle showing information dav can display on the OSD.
	...
	<li>A basic first configuration to consider is enabling motion detection to be turned on
		each time dav is started.  To do this, use the OSD menu system:<br>
		<ul>
			<li>
...
<div class='indent0'>
	...
	After an upgrade to a new version dav should be stopped
...
<div class='indent1' style='margin-top:8px;'>
	...
	<span style='font-weight:700'>dav</span> program which runs on the Pi Arm
	microprocessor.<br>
	</li>
	<p>
	<li>dav runs as user pi for simplified install and other benefits
	(but a default user other than pi is possible).
	PHP files are run via the web server nginx and so run as the user www-data.
	The install script created a <nobr><span style='font-weight:700'>/etc/sudoers.d/dav</span></nobr>
	file to give the user www-data permission to start dav as user pi from the web page.
	If the option to run dav at boot was selected, a line to start dav as user pi
	was added to <nobr><span style='font-weight:700'>/etc/rc.local</span></nobr>
	</li>
	<p>
	<li>The install script creates
	<span style='font-weight:700'>/etc/nginx/sites-available/dav</span> where listen port
	number and password protection for the web pages are configured.
	</li>
	<p>
	<li>Several dav internal control paths are not shown in the above diagram.
...
<span style='font-weight:700'>~/.dav/dav.conf</span>
...
for example, the vector array will be 120x68.  dav analyzes the vector array in
...
and counts against configured limits.  dav uses two methods in parallel to detect
motion:
...
<p>
You can see the results of dav's vector processing on the OSD by turning on
...
<ul>
	<li>Image 2 fails the overall motion detect because dav tries to be
	...
	<p>
	<li>Sparkles are camera motion vectors that have no neighbors and dav
...
<div class='indent0'>
dav has built in servo control for cameras mounted on servos.
<ul>
	<li><span style='font-weight:700'>Hardware PWM</span>: If servos are connected to
	the hardware PWM GPIO pins, dav can directly control the PWM signals and the
	only configuration needed is to set in dav.conf
	...
	Stop dav before editing ~/.dav/dav.conf to set the GPIO values.
	Then restart dav, reload the web page and you will have new buttons to control
	...
	<span style='font-weight:700'>servo_pan_gpio</span> and
	<span style='font-weight:700'>servo_tilt_gpio</span>
	in dav.conf to the ServoBlaster servo numbers you are using and set
	...
	documentation.<br>
	Stop dav before editing ~/.dav/dav.conf to set the use
	servoblaster option and gpio values to ServoBlaster servo numbers.
	Then restart dav and reload the web page.
	</li>
</ul>
...
<ul>
	<li> dav considers the camera at a single fixed position and will never be
...
<span style='font-size: 1.2em; font-weight: 650;'>Time Lapse</span>
	...
	<span style='font-weight:700'>End</span> button is clicked dav will run a time lapse
...
<a name="CONFIG_MOTION">
	<ul>
		<li><span style='font-weight:700'>Motion</span>
			...
			dav starts.  Motion detection can be
...
<p>
			...
			<nobr><span style='font-weight: 700;'>/run/dav/motion-events</span></nobr>
			file.
			</li>
			...
			event to start, dav requests key frames from the camera once per second,
...
<a name="DISKUSAGE">
		<li><span style='font-weight:700'>Loop</span><br>
			...
			be enabled each time dav starts.  Loop recording can be
...
<div class='indent0'>
dav can do two independent audio MP3 encodes.  One encode
...
then dav should work using the same sound device.  You may need to
...
<br>
So a microphone setup for dav is:
<ul>
	<li>
...
pi@rpi2: ~$ arecord  -D plughw:1 -d 5  test.wav
  # And also check using parameters the same as dav recording defaults,
...
pi@rpi2: ~$ aplay test.wav
	...
	audio_device in dav.conf to be the correct device
	and restart dav.
	</li>
...
<span style='font-size: 1.2em; font-weight: 680;'>Audio Parameters in dav.conf</span><br>
<div class='indent1'>
Edit dav.conf to change these settings:
...
<ul>
	...
	<li><span style='font-weight:700'>audio_channels</span> - default: 1<br>
		...
		be reverted to 1 when dav opens the microphone.
	</li>
...
microphone sensitivity</a> is likely needed for a dav application
...
<p>
The default loop directory is under the ~/dav/media directory.
This directory can be mounted with a dedicated loop file system and mounting
can be done in fstab or in the dav startup script.  Or a
dedicated disk can be mounted to some other directory if the loop_dir
value in dav.conf is edited to reference that location.
<p>
...
<pre>
echo "loop on" > ~/dav/www/FIFO
echo "loop off" > ~/dav/www/FIFO
echo "loop toggle" > ~/dav/www/FIFO
</pre>
...
<div class='indent0'>
<span style='font-size: 1.2em; font-weight: 650;'>~/.dav/dav.conf</span>
	<div class='indent1'>
	...
	If the file is manually edited, dav must be stopped and restarted to pick up the changes.
	...
	<ul>
		...
		the motion_area_min_side value in dav.conf should be correspondingly scaled.
		</li>
		<p>
		...
		detail in dav.conf where they are configured.
		</li>
		<p>
...
<p>
<span style='font-size: 1.2em; font-weight: 650;'>~/.dav/at-commands.conf</span>
	<div class='indent1'>
	This file is described below in its own section.
	</div>
<p>
<span style='font-size: 1.2em; font-weight: 650;'>~/dav/scripts/*</span>
	<div class='indent1'>
	Todo
	</div>
<p>
<span style='font-size: 1.2em; font-weight: 650;'>~/dav/www/config-user.php</span>
	<div class='indent1'>
	...
	dav distribution image or to your custom background image you place
	in the <nobr>~/dav/www/images</nobr> directory. Any custom image
...
<div class='indent0'>
dav media files are videos or jpegs which are stored in media directories.
...
automatically when dav runs and media files are stored in the media directory
sub directories.
<p>
...
that dav uses to store and view media files and the defaults are:
<pre>
pi@rpi2: ~$ ls -lt dav/www
total 116
lrwxrwxrwx 1 pi       www-data    33 Nov 16 09:07 archive -> /home/pi/dav/media/archive/
lrwxrwxrwx 1 pi       www-data    25 Nov 16 09:07 media -> /home/pi/dav/media/
</pre>
...
can be configured in <nobr>~/.dav/dav.conf</nobr>:
<pre>
media_dir media
archive_dir archive
</pre>
...
be stored on an external disk by editing the <nobr>~/dav/scripts/startup</nobr> file to uncomment
the line:
<pre>
MOUNT_DISK=sda1
</pre>
...
<nobr>/dev/sda1</nobr>, dav can create directories with the
needed permissions.  However, if the filesystem is not a linux filesystem
(eg. VFAT or FAT32) then dav cannot set up the needed permissions for
...
For this, use absolute pathnames in dav.conf:
<pre>
...
be needed in fstab so dav can make directories.  Look at the example
fstab entry forum
...
to move videos into a safer central location from one or more dav
camera installs.
...
<p>
By default, the archive directory is under the dav media directory
...
A network mount must have file system permissions set so that the dav
...
<div class='indent1'>
This is a minimal NFS setup for archiving from a Pi running dav to a
...
(one of my Pis running dav) and my LAN IP addresses.  The same setup
as for rpi5 can be done for other Pis running dav and then multiple
...
192.168.0.10    gkrellm6
	...
	<li>
	For this example I will mount onto the default dav archive location
	so I don't have to edit archive_dir in dav.conf.  I leave it
	at its default which assumes
	<nobr>/home/pi/dav/media/archive</nobr>
	...
	directory onto the dav archive directory. Use the archive_dir
	full path implied by the archive_dir value above:
<pre>
gkrellm6:/home/bill/media-archive /home/pi/dav/media/archive nfs users,noauto 0 0
</pre>
	</li>
	...
	<nobr>/home/bill/media-archive</nobr> directory on the dav
	<nobr>/home/pi/dav/media/archive</nobr> directory.
...
$ sudo mount gkrellm6:/home/bill/media-archive
	...
	<li> You can use the dav ~/dav/scripts/startup script
	to mount the NFS archive directory when dav starts.
	dav installs prior to V 4.1.5 did not have a NFS section in that
	startup script so you may have to copy the new
	<nobr>~/dav/scripts-dist/startup</nobr> over your existing
	<nobr>~/dav/scripts/startup.</nobr> and reconfigure
	MOUNT_DISK if you had previously done that.<br>
	Otherwise, configuration for NFS mounting must be as follows:
	<ul>
		<li> In ~/dav/scripts/startup, set NFS_ARCHIVE
		...
		<li> In dav.conf, set the on_startup command. The $a variable
		will be the archive_dir value configured in dav.conf and
...
<div class='indent1'>
This example is slightly more complicated and is my dav
archiving set up.  I have a USB SSD booted Pi3 desktop where I archive
videos from multiple Pis running dav.
My Pi3 desktop with a three partition USB SSD disk is rpi0,
and my Pis running dav are <nobr>rpi4, rpi5, ...</nobr>
...
<span style='font-weight:700'>On rpi4, rpi5, ...</span> (Pis running dav - archiving from)
<ul>
...
192.168.0.30    rpi0
	...
	<li> I don't want the dav archive directory left at the default
	location as <nobr>~/dav/media/archive</nobr> because
	I have all my dav media directories mounted with USB disks and
	I would rather not NFS mount into a USB mount.
	So, I set up similary to the structure I set up for rpi0 and
	have each dav archive to directory /mnt/archive/media-archive which
...
$ mkdir archive/media-archive
	...
	Stop dav and edit archive_dir in ~/.dav/dav.conf,
	then restart dav.
<pre>
...
$ sudo mount  rpi0:/mnt/archive/media-archive
	...
	<li> You can use the dav startup script to mount the NFS
	...
	<ul>
		...
		<li> In dav.conf, in addition to the archive_dir set as
...
the web server sends a command to dav to archive the files and the dav program
does the archiving.  Since dav runs as the user pi, dav has the sudo permission to
create the appropriate directory structure for archiving.
Since the web server runs as the user www-data, dav creates directories with write permission
for the user www-data so files can be deleted from the web page. All directories in the archive
path have permissions like:
<pre>

pi@rpi2: ~/dav/www/archive$ ls -Rl
.:
total 0
...
<pre>
echo "archive_video day 2017-11-13" > ~/dav/www/FIFO
</pre>
or a specific video (including its thumb) can be archived with:
<pre>
echo "archive_video motion_2017-11-05_14.46.14_456.mp4 2017-11-05" > ~/dav/www/FIFO
</pre>
To archive all media videos for today or yesterday:
<pre>
echo "archive_video day today" > ~/dav/www/FIFO
echo "archive_video day yesterday" > ~/dav/www/FIFO
</pre>
To archive all loop videos for today or yesterday:
<pre>
echo "archive_video day_loop today" > ~/dav/www/FIFO
echo "archive_video day_loop yesterday" > ~/dav/www/FIFO
</pre>
...
<div class='indent0'>
From the web page, command line or scripts, dav is controlled by sending commands to
a communication pipe named
<span style='font-weight:700'>FIFO</span> in the
<span style='font-weight:700'>~/dav/www</span> directory.
<p>
...
<pre>
echo "motion_enable [on|off|toggle]" > ~/dav/www/FIFO
</pre>
	</li>
	<li>
	Still jpegs are created when a still command is sent to the FIFO.
<pre>
echo "still" > ~/dav/www/FIFO"
</pre>
	</li>
...
<pre>
echo "record on" > ~/dav/www/FIFO
...
echo "record pause" > ~/dav/www/FIFO
...
echo "record on" > ~/dav/www/FIFO
...
echo "record off" > ~/dav/www/FIFO
</pre>
	</li>
...
<pre>
echo "record on 10 6" > ~/dav/www/FIFO
</pre>
	</li>
	<li>
<a name="MOTION_TRIGGER">
	...
	the <nobr>/run/dav/motion-events</nobr> file so an on_motion_begin
...
echo "motion trigger" > ~/dav/www/FIFO"
...
echo "motion trigger 1" > ~/dav/www/FIFO"
...
echo "motion trigger 0:PIR" > ~/dav/www/FIFO"
</pre>
	The second usage
...
<pre>
echo "motion trigger 1 4 5" > ~/dav/www/FIFO"
...
echo "motion trigger 1:laser 4 5" > ~/dav/www/FIFO"
</pre>
	</li>
	...
	<ul>
	<li>A bug around dav 2.0.0: deleting a thumb did not delete its video.
	</li>
...
<pre>
echo "fix_thumbs test" > ~/dav/www/FIFO
echo "fix_thumbs fix" > ~/dav/www/FIFO
</pre>
	</li>
...
<pre>
#!/bin/bash
echo annotate_text_background_color 808080 > ~/dav/www/FIFO
echo annotate_text_size 42 > ~/dav/www/FIFO
...
TEMP="29.1C"
echo annotate_string append id1 _$TEMP > ~/dav/www/FIFO; sleep 3
echo annotate_string prepend id2 ${HOSTNAME}_ > ~/dav/www/FIFO; sleep 3
TEMP="30.4C"
echo annotate_string append id1 _$TEMP > ~/dav/www/FIFO; sleep 3
echo annotate_string remove id2 > ~/dav/www/FIFO; sleep 3
echo annotate_string remove id1 > ~/dav/www/FIFO

echo annotate_text_background_color none > ~/dav/www/FIFO
echo inform \"End of annotate_string demo\" 3 3 1 > ~/dav/www/FIFO
echo inform timeout 3 > ~/dav/www/FIFO
</pre>
	</li>
...
<div class='indent0'>
dav has a built in command scheduler for periodic command execution at a particular
time.
The at commands are a list of commands in the configuration file
<span style='font-weight:700'>~/.dav/at-commands.conf.</span>
and if this file is modified, dav will automatically reload it so there is no
need to restart.
<p>
...
frequency  time   "command"
	...
	<li>
		...
		For all other times, the commands execute when the time occurs while dav
		...
		dav is started, you need to have your own script that checks the time.
		Such a script can parse the dav state file
		<span style='font-weight:700'>/run/dav/state</span>
		to get the
		...
		<span style='font-weight:700'>~/.dav/dav.conf</span> and set the
	...
	<li>
		...
		- a system command/script or an internal dav command if the command
	...
	Command strings may have substitution variables which are expanded by dav
	prior to being executed.  Substitution variables are:

<pre>
C - script commands directory full path
I - the dav install directory
...
daily    23:00  "$C/do-archive"
	...
	The ds18b20.py script is in the dav scripts directory and can be edited
...
<div class='indent0'>
<span style='font-size: 1.2em; font-weight: 700;'>/run/dav/state</span>
	<div class='indent1'>
	dav internal state can be read from this file.  Look at it to see
...
<pre>
line=`grep motion_enable /run/dav/state`
motion_state=${line#motion_enable}
	...
	state should show up in the /run/dav/state file
	within around 100-200 msec.
	</div>

<a name="MOTION_EVENTS">
<span style='font-size: 1.2em; font-weight: 700;'>/run/dav/motion-events</span>
	<div class='indent1'>
...
<div class='indent0'>
&nbsp;&nbsp;<span style='font-size: 1.2em; font-weight:700'>~/dav/www/motion_detects_FIFO</span>
<p>
This named pipe fifo is for near real time processing of all dav
...
<pre>
echo "motion_detects_fifo_enable [on|off|toggle]" > ~/dav/www/FIFO
</pre>
...
<nobr><span style='font-weight:700'>/run/dav/state</span></nobr>
file.
<p>
If the enable is "on" it will stay enabled across dav
restarts until an "off" command or dav.conf is edited.  Once enabled,
dav tries to write all motion detects into the motion_detects_FIFO
...
<span style='font-weight:700'>~/dav/scripts-dist/example-motion-detects-fifo</span>
for more information. If this script is run by hand from a terminal,
a stream of all motion detects is printed.
<p>
To test the example script on a Pi running dav, open a terminal
(SSH terminal if the Pi is headless) and run the example script:<br>

<pre>
    $ ~/dav/scripts-dist/example-motion-detects-fifo
...
#    $ echo "motion_detects_fifo_enable off" > ~/dav/www/FIFO
</pre>
Commands in at-commands.conf can coordinate times when you want
dav to have motion recordings and times when you might want to run
...
<div class='indent0'>
The multicast interface provides a means for davs and desktops
on a LAN to coordinate state and motion events by one to many message
sending and script running.
The coordination control
implementation can be distributed among the davs or the control can
be centralized on a desktop program that receives multicasts from the
davs and issues commands or messages back to them.
A system could be as simple as dav motion event alarm messages
...
(messages may be lost).  To address this, the dav multicast
...
programs that implement the dav LAN multicast system.
<p>
dav uses a fixed group network IP and port number for multicast
...
<ul>
	...
	for one or many davs on a LAN.
<pre>
# Turn off motion detection for all davs
pkc-motion all off
# or, "all" is assumed
pkc-motion off
...
pkc-reboot rpi1
	...
	multicast alarm message from a dav on your LAN.
	...
	<p>
	On each Pi you want dav to send alarm messages from, the setup is:
	<ol>
		...
		<nobr>/run/dav/motion-events</nobr> file
		...
		<li> Edit dav.conf and set the on_motion_begin command to:
<pre>on_motion_begin  $C/motion-send-alarm
	...
	<span style='font-weight:700'>pkc-recv</span> - run this in a terminal
		if you need to debug a dav  multicasting installation or
		development.  It just prints all the network traffic
		sent in the dav multicast group.
	</li>
</ul>
...
<ul>
	...
	all programs listening on the PKC multicast group.  Every dav
	...
	not interested in.  dav recognizes receiving message types
...
be interpreted as a command to execute by dav.
If the '@' char is prepended, the message body will be a dav
...
get dav to run a script that needs some internal variables passed
